...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:7),
                              to = c("Fibro", "Fibro", "Unknown", "Fibro", "Immune", 
                                     "Epi", "Epi"))
...
glial_cell_names <- static_choose_cell(proj, xlimit = c(-9.23, -8.03), ylimit = c(-10.17, -9.20))

tuned_idents <- getCellColData(proj, select = "tuned_ident")
tuned_idents[glial_cell_names, ] <- "Glial"
