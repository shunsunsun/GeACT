...
root <- opt$root
setwd(paste(root, "data/11-14w/10_bronchus/results", sep = "/"))
...
proj <- addClusters(proj, reducedDims = "peakLSI", name = "clusters", dimsToUse = 1:30, force = T, k.param = 15, prune.SNN = 1/15, resolution = 0.6)
...
proj$tuned_label <- mapvalues(proj$clusters, from = paste0("C", 1:7),
                              to = c("Fibroblast", "Fibroblast", "C3", "Fibroblast", 
                                     "C5", "Epithelial", "Epithelial"))
...
interactive_choose_cell(proj, "PLP1")
# interactive_choose_cell(proj)

cell_names <- static_choose_cell(proj, xlimit = c(-6.24, -4.61), ylimit = c(-9.26, -8.23))

tuned_labels <- getCellColData(proj, select = "tuned_label")
tuned_labels[cell_names, ] <- "Glial-like"
...
pdf("markers/FGC.pdf")
plotMarker(proj, genes = c("DDX4", "DAZL", "TDRD6", "PIWIL2", "POU5F1", "LIN28A", "SALL4", 
                           "DPPA5"), do_plot = F)
dev.off()

pdf("markers/granulosa.pdf")
plotMarker(proj, genes = c("WT1", "FOXL2"), do_plot = F)
dev.off()
