...
setwd(paste(root, "data/11-14w/12_esophagus/results", sep = "/"))
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:4),
                              to = c("Epi", "Epi", "Glial", "Fibro"))
...
# interactive_choose_cell(proj, "PECAM1")
# interactive_choose_cell(proj)

en_cell_names <- static_choose_cell(proj, xlimit = c(-33.3, -31.63), ylimit = c(-15.71, -14.05))

tuned_idents <- getCellColData(proj, select = "tuned_ident")
tuned_idents[en_cell_names, ] <- "Endo-like"
