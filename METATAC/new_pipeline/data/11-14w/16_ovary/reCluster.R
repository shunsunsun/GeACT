...
setwd(paste(root, "data/11-14w/16_ovary/results", sep = "/"))
...
proj <- addClusters(proj, reducedDims = "peakLSI", name = "clusters", dimsToUse = 1:30, force = T, k.param = 5, prune.SNN = 1/15, resolution = 0.2)
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:9),
                              to = c("Granulosa", "Granulosa", "Fibro", "Unknown", "Erythrocyte", 
                                     "Immune", "JDP2+", "BNIP3P15+", "Endo"))
...
# interactive_choose_cell(proj, "WT1")
...
plotMarker(proj, genes = c("WT1"), do_plot = F)
dev.off()

pdf("markers/others.pdf")
plotMarker(proj, genes = c("JDP2", "BNIP3P15"), do_plot = F)
dev.off()
