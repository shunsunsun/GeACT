...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:10),
                              to = c("Immune", "Immune", "Erythrocyte", "Endo", "Endo", 
                                     "Cardiomyocyte", "Glial", "Fibro", "Fibro", "Fibro"))
...
# interactive_choose_cell(proj, c("COL1A1", "HBB", "TYROBP", "CD3D"), group_bys = c("tuned_ident", "plate"))

un_cell_names <- static_choose_cell(proj, xlimit = c(0.29, 1.15), ylimit = c(0.88, 2.92), group_by = "tuned_ident", used_groups = "Fibro")
...
pdf("markers/CM.pdf")
plotMarker(proj, genes = c("TNNT2"), do_plot = F)
dev.off()
