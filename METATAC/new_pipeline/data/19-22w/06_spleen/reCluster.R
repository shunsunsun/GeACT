...
setwd(paste(root, "data/19-22w/06_spleen/results", sep = "/"))
...
proj <- addClusters(proj, reducedDims = "peakLSI", name = "clusters", dimsToUse = 1:30, force = T, k.param = 15, prune.SNN = 1/15, resolution = 0.4)
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:6),
                              to = c("Erythrocyte", "Erythrocyte", "T", "NKT", "Macro", 
                                     "B"))
...
# interactive_choose_cell(proj, c("HBB", "HBG1", "HBG2"), log2Norm = T, group_bys = "tuned_ident")
