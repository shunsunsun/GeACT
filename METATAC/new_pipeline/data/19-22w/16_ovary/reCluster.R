...
setwd(paste(root, "data/19-22w/16_ovary/results", sep = "/"))
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:5),
                              to = c("Unknown", "Unknown", "Granulosa", "Granulosa", "Granulosa"))
...
# interactive_choose_cell(proj, c("PECAM1"), log2Norm = T, group_bys = "tuned_ident")
# 
# un_cell_names  <- static_choose_cell(proj, xlimit = c(-0.17, 1.64), ylimit = c(-5.53, -3.36), group_by = "tuned_ident", used_groups = "Endo")
# 
...
plotMarker(proj, genes = c("WT1", "RGS1"), do_plot = F)
dev.off()
