...
root <- opt$root
setwd(paste(root, "data/19-22w/05_pancreas/results", sep = "/"))
...
proj$tuned_label <- mapvalues(proj$clusters, from = paste0("C", 1:11),
                              to = c("Epi", "Epi", "Epi", "Endo", "Glial", 
                                     "Fibro", "Fibro", "Erythrocyte", "Macro", "B", 
                                     "T"))
...
interactive_choose_cell(proj, c("EPCAM", "PLP1", "COL1A1", "COL2A1", "PECAM1", "MYH11"), log2Norm = T, group_bys = "tuned_label")

un_cell_names  <- static_choose_cell(proj, xlimit = c(0.26, 3.01), ylimit = c(-3.35, -0.86), group_by = "tuned_label", used_groups = "Fibro")

#TOCHECK, these epi-like cells
tuned_labels <- getCellColData(proj, select = "tuned_label")
tuned_labels[epi_cell_names, ] <- "Unknown"
