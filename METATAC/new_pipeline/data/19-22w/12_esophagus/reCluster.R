...
setwd(paste(root, "data/19-22w/12_esophagus/results", sep = "/"))
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:10),
                              to = c("Macro", "Immune", "Epi", "Unknown", "Endo", 
                                     "Glial", "Fibro", "Fibro", "Fibro", "Fibro"))
...
# interactive_choose_cell(proj, c("ACTG2", "COL1A1"), log2Norm = T, group_bys = "tuned_ident")

fibro_cell_names  <- static_choose_cell(proj, xlimit = c(-2.03, 2.09), ylimit = c(-1.69, 0.87), group_by = "tuned_ident", used_groups = "Endo")

tuned_idents <- getCellColData(proj, select = "tuned_ident")
tuned_idents[fibro_cell_names, ] <- "Fibro"
...
pdf("markers/DC.pdf")
plotMarker(proj, c("CD1C", "PLD4", "HLA-DRA", "CLEC9A", "LAMP3"), do_plot = F)
dev.off()
