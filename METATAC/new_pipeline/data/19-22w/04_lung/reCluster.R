...
setwd(paste(root, "data/19-22w/04_lung/results", sep = "/"))
...
proj <- addClusters(proj, reducedDims = "peakLSI", name = "clusters", dimsToUse = 1:30, force = T, k.param = 15, prune.SNN = 1/15, resolution = 0.15)
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:10),
                              to = c("Epi", "Fibro", "SM", "Pericyte", "Endo", 
                                     "Unknown", "T", "Erythrocyte", "Macro", "B"))
...
# interactive_choose_cell(proj, c("MARCO", "HBB", "HBG1"), log2Norm = T, group_bys = "tuned_ident")

ery_cell_names  <- static_choose_cell(proj, xlimit = c(10.42, 12.89), ylimit = c(1.56, 5.41), group_by = "tuned_ident", used_groups = "T")
un_cell_names <- static_choose_cell(proj, xlimit = c(0.86, 2.14), ylimit = c(9.21, 10.92), group_by = "tuned_ident", used_groups = "Pericyte")
