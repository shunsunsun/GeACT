...
proj$tuned_label <- mapvalues(proj$clusters, from = paste0("C", 1:9),
                              to = c("Epi", "Fibro", "SM", "Pericyte", "Endo", 
                                     "Unknown", "T", "B", "Erythrocyte"))
...
interactive_choose_cell(proj, "MARCO", log2Norm = T, group_bys = "tuned_label")

macro_cell_names  <- static_choose_cell(proj, xlimit = c(9.71, 12.97), ylimit = c(-8.25, -6.13))
un_cell_names <- static_choose_cell(proj, xlimit = c(-0.53, 1.43), ylimit = c(8.02, 11.31), group_by = "tuned_label", used_groups = "Pericyte")


tuned_labels <- getCellColData(proj, select = "tuned_label")
tuned_labels[macro_cell_names, ] <- "Macro"
tuned_labels[un_cell_names, ] <- "Unknown"
#? TOCHECK, C6中RBC的marker
#? TOCHECK, Smmoth muscle的边界
