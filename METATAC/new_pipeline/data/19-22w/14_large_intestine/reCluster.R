...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:9),
                              to = c("Epi", "Glial", "Fibro-like", "Fibro", "Fibro", 
                                     "SM", "T", "Endo", "Macro"))
...
# interactive_choose_cell(proj, c("PECAM1", "PLP1"), log2Norm = T, group_bys = "tuned_ident")

un_cell_names  <- static_choose_cell(proj, xlimit = c(-0.98, 0.36), ylimit = c(5.89, 8.81))
