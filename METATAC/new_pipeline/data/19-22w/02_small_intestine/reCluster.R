...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:12),
                              to = c("Macro", "T", "B", "Fibro", "Fibro", 
                                     "SM", "Fibro", "Fibro", "Fibro", "Epi", 
                                     "Glial", "Endo"))
...
# interactive_choose_cell(proj, c("EPCAM", "HBB", "HBG1", "PECAM1"), log2Norm = T, group_bys = "tuned_ident")

un_cell_names  <- static_choose_cell(proj, xlimit = c(-0.21, 3.93), ylimit = c(0.92, 5.07), group_by = "tuned_ident", used_groups = "Epi")
un_cell_names <- static_choose_cell(proj, xlimit = c(1.42, 2.11), ylimit = c(0.89, 1.47), group_by = "tuned_ident", used_groups = "Endo") %>% c(un_cell_names)

ery_cell_names <- static_choose_cell(proj, xlimit = c(1.65, 2.11), ylimit = c(0.62, 0.98), group_by = "tuned_ident", used_groups = "Endo")
