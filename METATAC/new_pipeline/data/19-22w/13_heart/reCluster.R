...
setwd(paste(root, "data/19-22w/13_heart/results", sep = "/"))
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:10),
                              to = c("Cardiomyocyte", "Glial", "Fibro", "Unknown", "Erythrocyte", 
                                     "Endo", "Unknown", "Unknown", "Immune-nonmacro", "Macro"))
...
# interactive_choose_cell(proj, c("PECAM1", "MS4A1", "ACTG2", "COL1A1"), log2Norm = T, group_bys = "tuned_ident")
# 
...
# tuned_idents[endo_cell_names, ] <- "Endo"
# tuned_idents[immune_cell_names, ] <- "Immune"
# 
...
pdf("markers/CM.pdf")
plotMarker(proj, "TNNT2", do_plot = F)
dev.off()

pdf("markers/epicadial.pdf")
plotMarker(proj, "WT1", do_plot = F)
dev.off()
