...
setwd(paste(root, "data/19-22w/07_testis/results", sep = "/"))
...
proj <- addClusters(proj, reducedDims = "peakLSI", name = "clusters", dimsToUse = 1:30, force = T, k.param = 10, prune.SNN = 1/15, resolution = 0.6)
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:8),
                              to = c("Unknown", "Fibro", "Fibro", "Sertoli", "Sertoli", 
                                     "Sertoli", "CALN1+", "Unknown"))
...
# interactive_choose_cell(proj, c("DAZL", "COL1A1", "MARCO"), log2Norm = T, group_bys = "tuned_ident")

#TOCHECK, these unknown cells
# un_cell_names  <- static_choose_cell(proj, xlimit = c(-6.93, -4.11), ylimit = c(-2.63, 0.23))
# 
...
pdf("markers/FGC.pdf")
plotMarker(proj, c("DAZL", "FGFR3"), do_plot = F)
dev.off()

pdf("markers/sertoli.pdf")
plotMarker(proj, c("TYMS", "CTSV", "WT1"), do_plot = F)
dev.off()

pdf("markers/others.pdf")
plotMarker(proj, c("CALN1"), do_plot = F)
dev.off()
