...
setwd(paste(root, "data/19-22w/15_liver/results", sep = "/"))
...
proj <- addClusters(proj, reducedDims = "peakLSI", name = "clusters", dimsToUse = 1:30, force = T, k.param = 20, prune.SNN = 1/15, resolution = 0.4)
...
proj$tuned_ident <- mapvalues(proj$clusters, from = paste0("C", 1:13),
                              to = c("Erythrocyte", "Epi", "Epi", "Hepatocyte", "Fibro", 
                                     "Endo", "Endo", "Macro", "Macro", "T", 
                                     "HMX1+", "B", "B"))
...
# interactive_choose_cell(proj, c("EPCAM", "HBB", "HBG1", "HBG2", "TYROBP", "MARCO"), log2Norm = T, group_bys = "tuned_ident")
# 
# interactive_choose_cell(proj, c("KRT5", "KRT14", "MUC5B", "MUC5AC"), log2Norm = T, group_bys = "tuned_ident")


un_cell_names <- static_choose_cell(proj, xlimit = c(6.99, 7.69), ylimit = c(-2.74, -2.16), group_by = "tuned_ident", used_groups = "Macro")
c1_cell_names <- static_choose_cell(proj, xlimit = c(1.13, 4.79), ylimit = c(-2.51, -0.27), group_by = "tuned_ident", used_groups = "Epi")
# un_cell_names <- static_choose_cell(proj, xlimit = c(-0.17, 1.64), ylimit = c(-5.53, -3.36), group_by = "tuned_ident", used_groups = "Epi?") %>% c(un_cell_names)

# t_cell_names <- static_choose_cell(proj, xlimit = c(4.23, 5.46), ylimit = c(18.03, 19.29), group_by = "tuned_ident", used_groups = "NKT")


tuned_idents <- getCellColData(proj, select = "tuned_ident")
tuned_idents[un_cell_names, ] <- "Unknown"
tuned_idents[c1_cell_names, ] <- "Unknown"

# tuned_idents[t_cell_names, ] <- "T"
...
plotMarker(proj, c("HMX1"), do_plot = F)
dev.off()
