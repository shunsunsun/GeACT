...
proj$tuned_label <- mapvalues(proj$clusters, from = paste0("C", 1:13),
...
interactive_choose_cell(proj, c("EPCAM", "HBB", "HBG1", "HBG2", "TYROBP", "MARCO"), log2Norm = T, group_bys = "tuned_label")

interactive_choose_cell(proj, c("KRT5", "KRT14", "MUC5B", "MUC5AC"), log2Norm = T, group_bys = "tuned_label")


un_cell_names <- static_choose_cell(proj, xlimit = c(3.85, 5.58), ylimit = c(0.96, 3.83), group_by = "tuned_label", used_groups = "Macro")
c1_cell_names <- static_choose_cell(proj, xlimit = c(-0.52, 3.21), ylimit = c(-1.57, 8.95), group_by = "tuned_label", used_groups = "Epi")
# un_cell_names <- static_choose_cell(proj, xlimit = c(-0.17, 1.64), ylimit = c(-5.53, -3.36), group_by = "tuned_label", used_groups = "Epi?") %>% c(un_cell_names)

# t_cell_names <- static_choose_cell(proj, xlimit = c(4.23, 5.46), ylimit = c(18.03, 19.29), group_by = "tuned_label", used_groups = "NKT")


tuned_labels <- getCellColData(proj, select = "tuned_label")
tuned_labels[un_cell_names, ] <- "Unknown"
tuned_labels[c1_cell_names, ] <- "Unknown"

# tuned_labels[t_cell_names, ] <- "T"
