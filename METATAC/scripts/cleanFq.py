...
META_table = dict()
for index in indexToSample:
    META_table[indexToSample[index]] = pd.DataFrame(0,
              index=['.']+[str(i) for i in range(1,len(META_sequence)+1)],
              columns=['.']+[str(i) for i in range(1,len(META_sequence)+1)])
...
while(1):
    ...
    if read1[2][0:R1_longest_len] in R1_index_dict and read2[2][0:R2_longest_len] in R2_index_dict:
        ...
        META_table[sample_ID].loc[str(M1),str(M2)] += 1
        
        read1[1] = '@'+str(M1)+'-'+str(M2)+'_'+read1[1][1:]
        read2[1] = '@'+str(M1)+'-'+str(M2)+'_'+read2[1][1:]
        
        for i in range(1,5):
            output_R1[sample_ID].write(read1[i])
            output_R2[sample_ID].write(read2[i])
        read_number[sample_ID] +=1
        pass_number += 1
f1.close()
f2.close()
...
for i in META_table:
    META_table[i].to_csv('/'.join([output_dir,ID,ID+'_'+i,ID+'_'+i+'_metaStat.csv']))