# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/09_bone_marrow/")
...
# curate a cluster
new_id <- 1
xxx_cells <- "GS_C-A2_87"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster
new_id <- 3
xxx_cells <- "GS_C-A3_70"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (CD34+)
new_id <- 4
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -3 & tSNE_1 < 0 & tSNE_2 > -21 & tSNE_2 < -17))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (TYMS+)
new_id <- 5
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -6.5 & tSNE_1 < -2.5 & tSNE_2 > -1 & tSNE_2 < 4))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (TYMS+)
new_id <- 6
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- setdiff(rownames(subset(xxx, tSNE_1 > -11 & tSNE_1 < -6 & tSNE_2 > 11 & tSNE_2 < 19)), c("GS_C-A3_91", "GS_C-A2_69", "GS_C-A3_19"))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (TYMS+)
new_id <- 7
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- setdiff(rownames(subset(xxx, tSNE_1 > 1 & tSNE_1 < 1.6 & tSNE_2 > 10 & tSNE_2 < 11.5)), c("GS_C-A3_91", "GS_C-A2_69", "GS_C-A3_19"))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (TYMS+)
new_id <- 2
xxx_cells <- rownames(subset(xxx, tSNE_1 > 4 & tSNE_1 < 9 & tSNE_2 > -20 & tSNE_2 < -6))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Pre-B", "Erythrocyte", "B-Prol", "Neutrophil", "Pro-B", "Mast", "Erythrocyte-Prol", "Neutrophil-Prol")
...
# change sort
cell_type_sorted <- c("Pro-B", "Pre-B", "B-Prol", "Mast", "Neutrophil", "Neutrophil-Prol", "Erythrocyte", "Erythrocyte-Prol")
...
###
p$layers[[3]]$data[5, 3] <- 8.8
p$layers[[3]]$data[6, 2] <- -0.8
p$layers[[3]]$data[6, 3] <- 9.5
p$layers[[3]]$data[8, 2] <- -9
p$layers[[3]]$data[8, 3] <- 14
###
p
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("GS_C"))
...
# dot plot
genes_list <- c("PTPRC", "CD79A", "CD34", "MS4A1", "TYMS", "CD63", "KIT", "S100A8", "TOP2A", "HBG1", "MKI67")
...
p$layers <- c(geom_rect(xmin = 1-0.5, xmax = 1+0.5, ymin = 3-0.5, ymax = 8+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 2-0.5, xmax = 2+0.5, ymin = 6-0.5, ymax = 8+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 8-0.5, xmax = 8+0.5, ymin = 3-0.5, ymax = 4+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 10-0.5, xmax = 10+0.5, ymin = 1-0.5, ymax = 2+0.5, fill = "skyblue"), p$layers)
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -7.5, r = 12.5, b = 15)), strip.background = element_blank()) + 
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(0, cumsum(tmp_DF$value)[6])),
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("GS"))
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  #           aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(1.5, 1.5), y = 1 - c(0, cumsum(tmp_DF_sub$value)[6])), 
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
