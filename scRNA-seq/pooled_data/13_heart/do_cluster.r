...
# curate a cluster (Fibro-GPC3)
new_id <- 1
xxx_cells <- "ZDM_D-A4_89"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (Endo)
new_id <- 5
xxx_cells <- "ZDM_D-A5_05"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (SM-Vascular)
new_id <- 9
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -15 & tSNE_1 < -12 & tSNE_2 > 0 & tSNE_2 < 5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Fibro-Prol)
new_id <- 10
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -8 & tSNE_1 < -3 & tSNE_2 > 1 & tSNE_2 < 5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (RC)
new_id <- 11
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 2 & tSNE_1 < 4 & tSNE_2 > 4 & tSNE_2 < 6))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (CACNA1A)
new_id <- 12
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 6.5 & tSNE_1 < 13.5 & tSNE_2 > 5 & tSNE_2 < 12))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (B)
new_id <- 13
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 20 & tSNE_1 < 23 & tSNE_2 > -24 & tSNE_2 < -19))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Mast)
new_id <- 14
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 23 & tSNE_1 < 24 & tSNE_2 > -18 & tSNE_2 < -17))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Mast)
new_id <- 15
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- setdiff(rownames(subset(xxx, tSNE_1 > 26 & tSNE_1 < 30 & tSNE_2 > -16 & tSNE_2 < -11)), c("FDM_D-C2_56", "FDM_D-C4_84"))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro-ADAMTS19", "Fibro-ITM2A", "SM-ACTG2", "T", "Fibro-FMOD", "Endo", "Glial", "Fibro-HTR2A", "DC/Macro", "SM-NOTCH3", 
                     "Fibro-Prol", "Erythrocyte", "CACNA1A", "B", "Mast", "NKT")
...
# change sort
cell_type_sorted <- c("Endo", "SM-ACTG2", "SM-NOTCH3", "Fibro-ADAMTS19", "Fibro-FMOD", "Fibro-ITM2A", "Fibro-HTR2A", "Fibro-Prol", 
                      "Glial", "B", "DC/Macro", "Mast", "NKT", "T", "CACNA1A", "Erythrocyte")
...
# short name
enrich_res_DF$Term <- gsub(", .*", "", enrich_res_DF$Term)
...
gp <- ggplot(enrich_res_DF, aes(x = Term, y = -log10(Adjusted.P.value), fill = new)) + 
  ...
  scale_fill_manual(values = scales::hue_pal()(length(levels(enrich_res_DF$new)) + 1)[-1], drop = F) + 
...
p <- TSNEPlot(object = tmp, do.label = TRUE, pt.size = 1, label.size = 3, no.legend = F, do.return = T) +
  scale_color_manual(labels = ident_labels, values = scales::hue_pal()(length(ident_labels) + 1)[-1]) + 
  theme(aspect.ratio = 1, axis.line = element_line(color = "black"), panel.border = element_blank()) + 
  #guides(color = guide_legend(ncol = 1)) + 
  coord_cartesian(clip = "off") + 
  xlab("tSNE-1") + ylab("tSNE-2")
###
p$layers[[3]]$data[3, 2] <- -14
p$layers[[3]]$data[3, 3] <- 0
p$layers[[3]]$data[8, 3] <- 2.5
p$layers[[3]]$data[12, 2] <- 22.5
p$layers[[3]]$data[12, 3] <- -15.5
###
p
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("BM_D", "ZDM_D", "FDM_D"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("Valve", "Aorta", "PA")
  levels(tmp@meta.data$bigBatch)
  ###
...
# dot plot
genes_list <- c("PECAM1", "ACTA2", "ACTG2", "NOTCH3", "COL1A1", "ADAMTS19", "FMOD", "ITM2A", "HTR2A", "TYMS", 
                "S100B", "PTPRC", "CD79A", "CD68", "KIT", "KLRB1", "CD3D", "CACNA1A", "HBG1")
p <- DotPlot_new(object = expr_assigned, genes.plot = genes_list, 
  ...
  theme(legend.box.margin = margin(r = 25)) + 
  theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5, linetype = 1))
p$layers <- c(geom_rect(xmin = 2-0.5, xmax = 2+0.5, ymin = 14-0.5, ymax = 15+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 5-0.5, xmax = 5+0.5, ymin = 9-0.5, ymax = 13+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 12-0.5, xmax = 12+0.5, ymin = 3-0.5, ymax = 7+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp, aes(x = new, y = S.Score + G2M.Score, fill = new)) + geom_boxplot(show.legend = F) + 
  ...
  scale_fill_manual(values = scales::hue_pal()(length(levels(tmp$new)) + 1)[-1], drop = F)
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -12.5, r = 5, b = 15)), strip.background = element_blank()) + 
...
# fraction (pooled)
tmp <- table(subset(cellMetaDatax, T, "ident", drop = T), useNA = "al")
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[9], cumsum(tmp_DF$value)[14])),
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("BM", "ZDM", "FDM"))

### change levels
levels(tmp_DF$bigBatch)
levels(tmp_DF$bigBatch) <- c("Valve", "Aorta", "PA")
levels(tmp_DF$bigBatch)
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 1+0.8/2, xend = 2-0.8/2, y = (1-cumsum(Valve)), yend = (1-cumsum(Aorta))), size = 0.3) + 
  geom_segment(mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = 1, yend = 1), size = 0.3) + 
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = (1-cumsum(Aorta)), yend = (1-cumsum(PA))), size = 0.3) + 
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Fibro-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Fibro-", tmp_DF_sub$ident))[1]])),
            aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) +
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF$value)[9], cumsum(tmp_DF_sub$value)[14])),
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) +
...
DoHeatmap_new(object = expr_assigned, genes.use = tmp$gene, genes.group = tmp$ident, slim.col.label = TRUE, remove.key = F, rotate.key = T, 
              group.cex = 11, cex.row = 7, do.colBar = T, colBar.col = scales::hue_pal()(length(levels(expr_assigned@ident)) + 1)[-1], strip.text.x.top = 15, 
              panel.spacing.y = 0, 
...
DoHeatmap_new(object = expr_assigned, genes.use = cyc_DF$gene, genes.group = factor(cyc_DF$stage, levels = c("S", "G2/M")), slim.col.label = TRUE, remove.key = F, rotate.key = T, 
              group.cex = 11, cex.row = 7, do.colBar = T, colBar.col = scales::hue_pal()(length(levels(expr_assigned@ident)) + 1)[-1], switch_y = T, strip.text.x.top = 15, 
              col.low = "blue", col.mid = "white", col.high = "red",
