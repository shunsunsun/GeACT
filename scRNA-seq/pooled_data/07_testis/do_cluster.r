# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/07_testis/")
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > -23 & tSNE_1 < -17 & tSNE_2 > -4 & tSNE_2 < 2))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 8
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -28 & tSNE_1 < -27 & tSNE_2 > -2 & tSNE_2 < -1))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 9
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -27 & tSNE_1 < -22 & tSNE_2 > -16.5 & tSNE_2 < -13.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro-FOSB+", "Fibro-FOSB-", "Sertoli-EGR3+", "SSC", "Sertoli-EGR3-", "Fibro-Prol", "Endo", "Immune", "Erythrocyte", "Sertoli-Prol")
...
# change sort
cell_type_sorted <- c("SSC", 
                      "Endo", 
                      "Fibro-FOSB+", "Fibro-FOSB-", "Fibro-Prol", "Sertoli-EGR3+", "Sertoli-EGR3-", "Sertoli-Prol", 
                      "Immune", "Erythrocyte")
...
###
p$layers[[3]]$data[5, 2] <- -11
p$layers[[3]]$data[5, 3] <- -2
p$layers[[3]]$data[9, 3] <- 0
p$layers[[3]]$data[10, 2] <- -28
p$layers[[3]]$data[10, 3] <- -2.4
###
p
...
# dot plot
genes_list <- c("FGFR3", "PECAM1", 
                "COL1A1", "FOSB", "TYMS", "CTSV", "EGR3", "TOP2A", "PTPRC", "HBG1")
...
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 6-0.5, ymax = 8+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 3-0.5, ymax = 5+0.5, fill = "skyblue"), p$layers)
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -10, r = 12.5, b = 15)), strip.background = element_blank()) + 
...
# fraction (pooled)
tmp <- table(factor(subset(cellMetaDatax, ! ident %in% c("SSC"), "ident", drop = T)), useNA = "al")
...
cellMetaDatas <- subset(cellMetaDatax, ! ident %in% c("SSC"), c("bigBatch", "ident"))
cellMetaDatas$ident <- factor(cellMetaDatas$ident)
cellMetaDatas$bigBatch <- gsub("[NP]_[A-Z]$", "", as.character(cellMetaDatas$bigBatch))
tmp <- table(cellMetaDatas)
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("JC"))
