# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/06_spleen/")
...
# curate a cluster (DC/Macro)
new_id <- 6
xxx_cells <- rownames(subset(xxx, tSNE_1 > 12 & tSNE_1 < 24 & tSNE_2 > -16 & tSNE_2 < -9))
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > -22 & tSNE_1 < -18 & tSNE_2 > 14 & tSNE_2 < 20))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (RC-Prol)
new_id <- 8
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -18 & tSNE_1 < -14 & tSNE_2 > 12 & tSNE_2 < 18))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (B-Prol)
new_id <- 9
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -6 & tSNE_1 < -2 & tSNE_2 > -2.5 & tSNE_2 < 2.2))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("B-CD83", "NKT", "T-TNFAIP3", "B-HLA-DRB5", "T-GIMAP7", "MPP", "DC/Macro", "Erythrocyte", "Erythrocyte-Prol", "B-Prol")
...
# change sort
cell_type_sorted <- c("MPP", 
                      "B-CD83", "B-HLA-DRB5", "B-Prol", "DC/Macro", "NKT", "T-GIMAP7", "T-TNFAIP3", 
                      "Erythrocyte", "Erythrocyte-Prol")
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("PZ_C", "PZ_D"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("C", "D")
  levels(tmp@meta.data$bigBatch)
  ###
...
# dot plot
genes_list <- c("CD34", "PTPRC", "CD79A", "CD83", "HLA-DRB5", "TYMS", "CD68", 
                "KLRB1", "CD3D", "GIMAP7", "TNFAIP3", "HBG1", "TOP2A")
...
p$layers <- c(geom_rect(xmin = 2-0.5, xmax = 2+0.5, ymin = 3-0.5, ymax = 9+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 7-0.5, ymax = 9+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 9-0.5, xmax = 9+0.5, ymin = 3-0.5, ymax = 5+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 12-0.5, xmax = 12+0.5, ymin = 1-0.5, ymax = 2+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(0, cumsum(tmp_DF$value)[8])),
...
tmp <- table(cellMetaDatas[, c("bigBatch", "ident")])
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("PZ"))
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  #           aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(1.5, 1.5), y = 1 - c(0, cumsum(tmp_DF_sub$value)[8])),
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) +
