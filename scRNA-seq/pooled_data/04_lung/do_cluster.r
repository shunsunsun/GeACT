# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/04_lung/")
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > -37 & tSNE_1 < -36 & tSNE_2 > -18 & tSNE_2 < -16))
...
new.cluster.ids <- c("Fibro-0", "Fibro-FOSB", "SM-STC1", "SM-HHIP", "Epi", "Pericyte", "Fibro-Prol", "Fibro-TNC", "Endo", "Immune", 
                     "Erythrocyte")
...
# change sort
cell_type_sorted <- c("Epi", "Endo", 
                      "SM-STC1", "SM-HHIP", "Fibro-0", "Fibro-FOSB", "Fibro-TNC", "Fibro-Prol", "Pericyte", 
                      "Immune", "Erythrocyte")
cell_type_sorted
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("FZY_C", "FZW_C"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("Central", "Surrounding")
  levels(tmp@meta.data$bigBatch)
  ###
