...
samplingPos <- "."
OUT <- paste0("03-expression/merged/cellCluster_adj/", samplingPos)
...
xxx_cells <- intersect(names(expr@ident)[expr@ident == 4], rownames(subset(xxx, tSNE_1 < 2)))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (B)
new_id <- 16
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -43 & tSNE_1 < -40 & tSNE_2 > -5 & tSNE_2 < -2))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (DC/Macro)
new_id <- 17
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -46 & tSNE_1 < -43 & tSNE_2 > -4 & tSNE_2 < -1))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 18
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- intersect(names(expr@ident)[expr@ident == 8], rownames(subset(xxx, tSNE_1 > 13)))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 19
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -8.3 & tSNE_1 < -7.9 & tSNE_2 > 36.8 & tSNE_2 < 37.4))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro-ADAM28", "Fibro-FBLN1", "Fibro-IRF1", "Fibro-Prol", "Fibro-KCNJ8", "SM-Visceral", "Fibro-VSTM2A", "T", "CACNA1A-ELF3-", "Fibro-NRK", 
                     "Endo", "Epi", "Glial", "SM-Vascular", "Erythrocyte", "Fibro-PRSS35", "B", "DC/Macro", "CACNA1A-ELF3+", "Fibro-KIT")
...
cell_type_sorted <- c("Epi","Endo","SM-Visceral","SM-Vascular",
                      "Fibro-KCNJ8","Fibro-ADAM28","Fibro-IRF1","Fibro-VSTM2A","Fibro-FBLN1","Fibro-NRK","Fibro-KIT","Fibro-PRSS35","Fibro-Prol",
                      "Glial","B","DC/Macro","T","CACNA1A-ELF3+","CACNA1A-ELF3-","Erythrocyte")
cell_type_sorted
...
pdf(file = paste0(OUT, "/", samplingPos, "/enrichment_all.pdf"), width = 8, height = 9.5)
...
ggsave(filename = paste0(OUT, "/GOenrich_comparison.pdf"), plot = gp, width = 6, height = 4.5, useDingbats = F)
...
gp <- gp + geom_hline(yintercept = c(11.5, 13.5), linetype = "dashed", color = "grey70")
...
p$layers[[3]]$data[15, 2:3] <- list(-42.25, -6)
p$layers[[3]]$data[16, 2:3] <- list(-41, 0)
p$layers[[3]]$data[18, 2:3] <- list(21, -33)
p$layers[[3]]$data[19, 3] <- -27
###
p
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("WD_C", "WT_C", "WDOU_C"))
...
p <- DotPlot_new(object = expr_assigned, genes.plot = genes_list, 
  ...
  theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5, linetype = 1))
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 17-0.5, ymax = 18+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 8-0.5, ymax = 16+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 17-0.5, xmax = 17+0.5, ymin = 4-0.5, ymax = 6+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 21-0.5, xmax = 21+0.5, ymin = 2-0.5, ymax = 3+0.5, fill = "skyblue"), p$layers)
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("WD_C", "WT_C", "WDOU_C"))
...
pdf(paste0(OUT, "/Seurat_tSNE_assigned_big.pdf"), width = 8, height = 7, useDingbats = F) # 6*6
