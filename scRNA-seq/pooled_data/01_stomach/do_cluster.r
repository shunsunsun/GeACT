...
new.cluster.ids <- c("Fibro-ADAM28", "Fibro-FBLN1", "Fibro-IRF1", "Fibro-Prol", "Fibro-KCNJ8", "SM-Visceral", "Fibro-VSTM2A", "T", "CACNA1A-ELF3-", "Fibro-NRK", 
                     "Endo", "Epi", "Glial", "SM-Vascular", "Erythrocyte", "Fibro-PRSS35", "B", "DC/Macro", "CACNA1A-ELF3+")
...
cell_type_sorted <- c("Epi","Endo","SM-Visceral","SM-Vascular",
                      "Fibro-KCNJ8","Fibro-ADAM28","Fibro-IRF1","Fibro-VSTM2A","Fibro-FBLN1","Fibro-NRK","Fibro-PRSS35","Fibro-Prol",
...
genes_list <- c("EPCAM", "PECAM1", "ACTA2", "ACTG2", "NOTCH3", 
                "PDGFRA", "KCNJ8", "ADAM28", "IRF1", "VSTM2A", "FBLN1", "NRK", "PRSS35", "TYMS", "PLP1", "PTPRC", "CD79A", "CD68", "CD3D", "CACNA1A", "ELF3", "HBG1")
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[13], cumsum(tmp_DF$value)[16])),
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Fibro-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Fibro-", tmp_DF_sub$ident))[1]])), 
            aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[13], cumsum(tmp_DF_sub$value)[16])), 
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
