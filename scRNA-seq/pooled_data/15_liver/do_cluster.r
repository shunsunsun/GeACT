# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/15_liver/")
...
# curate a cluster (NKT)
new_id <- 9
xxx_cells <- rownames(subset(xxx, tSNE_1 > 34 & tSNE_1 < 40 & tSNE_2 > -13 & tSNE_2 < -7))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (X)
new_id <- 11
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -6 & tSNE_1 < -3 & tSNE_2 > -34 & tSNE_2 < -29))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Fibro)
new_id <- 12
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 2 & tSNE_1 < 3 & tSNE_2 > -34 & tSNE_2 < -32))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (LSEC)
new_id <- 13
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -13 & tSNE_1 < -11 & tSNE_2 > -32 & tSNE_2 < -30))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (MPP)
new_id <- 14
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -25 & tSNE_1 < -10.2 & tSNE_2 > -29 & tSNE_2 < -17))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Hepatocyte)
new_id <- 15
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -2 & tSNE_1 < 0 & tSNE_2 > -18 & tSNE_2 < -16))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Endo)
new_id <- 16
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -6 & tSNE_1 < -3 & tSNE_2 > -8 & tSNE_2 < -4))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (NKT-Prol)
new_id <- 17
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 26 & tSNE_1 < 31 & tSNE_2 > -12 & tSNE_2 < -4))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("GZS4_D", "GZS5_D", "GZS8_D", "GZ7623_D"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("IV", "V", "VIII", "Around")
  levels(tmp@meta.data$bigBatch)
  ###
...
# dot plot
genes_list <- c("EPCAM", "PECAM1", "DNTT", "DNASE1L3", "TYMS", "COL1A1", 
                "CD34", "PTPRC", "CD79A", "MS4A1", "TOP2A", "CD68", "CPA3", "KLRB1", "IL2RB", "SYNE1", "MKI67", "CD3D", 
                "CYP3A7", "HBG1", "KIF2C")
...
p$layers <- c(geom_rect(xmin = 2-0.5, xmax = 2+0.5, ymin = 15-0.5, ymax = 17+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 8-0.5, xmax = 8+0.5, ymin = 4-0.5, ymax = 12+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 9-0.5, xmax = 9+0.5, ymin = 10-0.5, ymax = 12+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 14-0.5, xmax = 14+0.5, ymin = 5-0.5, ymax = 7+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 20-0.5, xmax = 20+0.5, ymin = 1-0.5, ymax = 2+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[1], cumsum(tmp_DF$value)[4])),
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[6], cumsum(tmp_DF$value)[15])),
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("GZS4", "GZS5", "GZS8", "GZ7623"))

### change levels
levels(tmp_DF$bigBatch)
levels(tmp_DF$bigBatch) <- c("IV", "V", "VIII", "Around")
levels(tmp_DF$bigBatch)
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_segment(mapping = aes(x = 3+0.8/2, xend = 4-0.8/2, y = 1, yend = 1), size = 0.3) + 
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 3+0.8/2, xend = 4-0.8/2, y = (1-cumsum(VIII)), yend = (1-cumsum(Around))), size = 0.3) + 
  geom_path(data = data.frame(x = c(4.5, 4.5), y = 1 - c(cumsum(tmp_DF_sub$value)[1], cumsum(tmp_DF_sub$value)[4])),
            aes(x = x, y = y), color = "orange", size = 1.5, inherit.aes = F) + 
  ...
  #           aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) +
  geom_path(data = data.frame(x = c(4.5, 4.5), y = 1 - c(cumsum(tmp_DF_sub$value)[6], cumsum(tmp_DF_sub$value)[15])),
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
