# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/14_large_intestine/")
...
# curate a cluster (CACNA1A)
new_id <- 3
xxx_cells <- "HJC_D-C4_93"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (SM-Vascular)
new_id <- 6
xxx_cells <- "SJC_D-A3_22"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (DC)
new_id <- 7
xxx_cells <- "SJC_D-A5_60"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (SM-Visceral)
new_id <- 8
xxx_cells <- "JJC_D-A3_71"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (DC)
new_id <- 9
xxx_cells <- "JJC_D-A2_61"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Fibro-Prol)
new_id <- 10
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -6.5 & tSNE_1 < -3 & tSNE_2 > -6 & tSNE_2 < -3))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Glial)
new_id <- 11
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -4.5 & tSNE_1 < -2 & tSNE_2 > 1 & tSNE_2 < 5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Epi)
new_id <- 12
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -7 & tSNE_1 < -4 & tSNE_2 > 9.5 & tSNE_2 < 12.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (RC)
new_id <- 13
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -5.5 & tSNE_1 < -4 & tSNE_2 > 15 & tSNE_2 < 18))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Mast)
new_id <- 14
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 6.5 & tSNE_1 < 8.5 & tSNE_2 > 21.5 & tSNE_2 < 22.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro-VCAN", "Fibro-ADAMDEC1", "Fibro-DPT", "CACNA1A", "T", "Fibro-BMP5", "SM-Vascular", "DC/Macro", "SM-Visceral", "Endo", 
                     "Fibro-Prol", "Glial", "Epi", "Erythrocyte", "Mast")
...
# change sort
cell_type_sorted <- c("Epi", "Endo", "SM-Visceral", "SM-Vascular", "Fibro-VCAN", "Fibro-DPT", "Fibro-ADAMDEC1", "Fibro-BMP5", "Fibro-Prol", "Glial", "DC/Macro", "Mast", "T", "CACNA1A", "Erythrocyte")
...
###
p$layers[[3]]$data[3, 2] <- -22
p$layers[[3]]$data[11, 2] <- 5.5
p$layers[[3]]$data[12, 3] <- 20.5
###
p
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("SJC_D", "HJC_D", "JJC_D"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("AC", "TC", "DC")
  levels(tmp@meta.data$bigBatch)
  ###
...
genes_list <- c("EPCAM", "PECAM1", "ACTA2", "ACTG2", "NOTCH3", 
                "COL1A1", "VCAN", "DPT", "ADAMDEC1", "BMP5", "TYMS", "S100B", 
                "PTPRC", "CD68", "KIT", "CD3D", "CACNA1A", "HBG1")
...
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 12-0.5, ymax = 13+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 7-0.5, ymax = 11+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 13-0.5, xmax = 13+0.5, ymin = 3-0.5, ymax = 5+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[10], cumsum(tmp_DF$value)[13])),
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("SJC", "HJC", "JJC"))

### change levels
levels(tmp_DF$bigBatch)
levels(tmp_DF$bigBatch) <- c("AC", "TC", "DC")
levels(tmp_DF$bigBatch)
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 1+0.8/2, xend = 2-0.8/2, y = (1-cumsum(AC)), yend = (1-cumsum(TC))), size = 0.3) + 
  geom_segment(mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = 1, yend = 1), size = 0.3) +
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = (1-cumsum(TC)), yend = (1-cumsum(DC))), size = 0.3) +
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Fibro-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Fibro-", tmp_DF_sub$ident))[1]])),
            aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) +
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[10], cumsum(tmp_DF_sub$value)[13])),
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
