# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/16_ovary/")
...
# curate a cluster
new_id <- 5
xxx_cells <- "RCP_C-A4_42"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster
new_id <- 6
xxx_cells <- "RCP_C-A5_56"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (WT1+DDX4+)
new_id <- 9
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -9.5 & tSNE_1 < -7.5 & tSNE_2 > 32 & tSNE_2 < 39))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (RC)
new_id <- 10
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- "RCP_C-A8_32"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (E/M)
new_id <- 11
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 23 & tSNE_1 < 26 & tSNE_2 > -8.5 & tSNE_2 < -6.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Endo)
new_id <- 12
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- "RCP_C-D7_73"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Glial)
new_id <- 13
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- "RCP_C-B8_94"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Prol)
new_id <- 14
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 11 & tSNE_1 < 16 & tSNE_2 > -23 & tSNE_2 < -20))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Prol-KRT19)
new_id <- 15
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 26 & tSNE_1 < 28 & tSNE_2 > -17 & tSNE_2 < -15))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Granulosa-R-Al", "Granulosa-R-Am", "Granulosa-R-Ah", "Granulosa-R+", "FGC", "Granulosa-R-CALB2", "Fibro-VCAN", "CACNA1A", "Immune", "FGC/Granulosa", 
                     "Erythrocyte", "Epi/Mes", "Endo", "Glial", "Granulosa-Prol", "Fibro-CCDC80")
...
# change sort
cell_type_sorted <- c("FGC", "FGC/Granulosa", "Epi/Mes", 
                      "Endo", "Fibro-VCAN", "Fibro-CCDC80", "Glial", "Immune", 
                      "Granulosa-R+", "Granulosa-R-Ah", "Granulosa-R-Am", "Granulosa-R-Al", "Granulosa-R-CALB2", "Granulosa-Prol", 
                      "CACNA1A", "Erythrocyte")
...
p <- TSNEPlot(object = tmp, do.label = TRUE, pt.size = 1, label.size = 3, no.legend = F, do.return = T) +
  ...
  annotate("segment", x = 30, xend = 33, y = -10, yend = -8, color = scales::hue_pal()(length(levels(tmp@ident)))[7])
###
p$layers[[3]]$data[2, 3] <- 33
p$layers[[3]]$data[3, 2] <- 20
p$layers[[3]]$data[4, 2:3] <- c(29.2, -7)
p$layers[[3]]$data[5, 2:3] <- c(36.5, -4.5)
p$layers[[3]]$data[6, 2:3] <- c(35, -15)
p$layers[[3]]$data[7, 2:3] <- c(27.2, -11)
p$layers[[3]]$data[13, 2:3] <- c(28, -19)
p$layers[[3]]$data[14, 3] <- -23.5
p$layers[[3]]$data[14, 3] <- -23.5
p$layers[[3]]$data[15, 3] <- 22
p$layers[[3]]$data[16, 2:3] <- c(29, 17)
###
p
...
# dot plot
genes_list <- c("DAZL", "EPCAM", "VIM", "CD24", "PECAM1", "COL1A1", "VCAN", "CCDC80", "S100B", "PTPRC", 
                "WT1", "RDH10", "ARL5B", "CALB2", "TYMS", "CACNA1A", "HBG1")
...
p$layers <- c(geom_rect(xmin = 1-0.5, xmax = 1+0.5, ymin = 15-0.5, ymax = 16+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 11-0.5, ymax = 12+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 11-0.5, xmax = 11+0.5, ymin = 3-0.5, ymax = 8+0.5, fill = "skyblue"), p$layers)
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  scale_fill_manual(values = rev(scales::hue_pal()(length(levels(tmp_data_melted$ident)))), drop = F) + 
  scale_color_manual(values = rev(scales::hue_pal()(length(levels(tmp_data_melted$ident)))), drop = F) + 
  theme(axis.title = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.line.x = element_blank()) + 
  theme(panel.grid = element_blank()) + 
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -12.5, r = 10, b = 15)), strip.background = element_blank()) + 
...
ggplot(tmp_DF, aes(x = ident, y = value, fill = ident)) + 
  ...
  scale_fill_manual(values = scales::hue_pal()(length(levels(tmp_DF$ident)) + 2)[-c(1, 2)], drop = F)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  # geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[5], cumsum(tmp_DF$value)[14])),
  #           aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[6], cumsum(tmp_DF$value)[12])),
            aes(x = x, y = y), color = "lightpink", size = 1.5, inherit.aes = F) +
...
cellMetaDatas <- subset(cellMetaDatax, ! ident %in% c("FGC", "FGC/Granulosa"), c("bigBatch", "ident"))
cellMetaDatas$ident <- factor(cellMetaDatas$ident)
cellMetaDatas$bigBatch <- gsub("_[A-Z]$", "", as.character(cellMetaDatas$bigBatch))
tmp <- table(cellMetaDatas)
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("RCP"))
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  #           aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) +
  geom_path(data = data.frame(x = c(1.5, 1.5), y = 1 - c(cumsum(tmp_DF_sub$value)[6], cumsum(tmp_DF_sub$value)[12])),
            aes(x = x, y = y), color = "lightpink", size = 1.5, inherit.aes = F) + 
...
tmp <- tmp[! duplicated(tmp$gene), ]
tmp <- do.call("rbind", split(tmp, tmp$ident))
DoHeatmap_new(object = expr_assigned, genes.use = c(tmp$gene, "HBG1", "HBG2", "HBB"), genes.group = factor(c(as.character(tmp$ident), rep("Erythrocyte", 3)), levels = levels(tmp$ident)), slim.col.label = TRUE, remove.key = F, rotate.key = T, 
