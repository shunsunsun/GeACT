# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/12_esophagus/")
...
# curate a cluster (Fibro)
new_id <- 0
xxx_cells <- c(rownames(subset(xxx, tSNE_1 > -2.5 & tSNE_1 < 2 & tSNE_2 > -5.5 & tSNE_2 < -3.5)), "SD_D-A3_79")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# curate a cluster (Epi)
new_id <- 3
xxx_cells <- c(rownames(subset(xxx, tSNE_1 > 3.5 & tSNE_1 < 8 & tSNE_2 > -6 & tSNE_2 < -1.5)), "SD_D-A2_03")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Fibro-Prol)
new_id <- 5
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 2.5 & tSNE_1 < 4 & tSNE_2 > 5.5 & tSNE_2 < 8))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Glial)
new_id <- 6
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 5.6 & tSNE_1 < 6.6 & tSNE_2 > 3 & tSNE_2 < 5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Immune)
new_id <- 7
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 8.5 & tSNE_1 < 12 & tSNE_2 > 5 & tSNE_2 < 7))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (RC)
new_id <- 8
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 8 & tSNE_1 < 9 & tSNE_2 > 0 & tSNE_2 < 1))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Endo)
new_id <- 9
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -13.5 & tSNE_1 < -11 & tSNE_2 > 4.5 & tSNE_2 < 7.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Stem)
new_id <- 10
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- c(rownames(subset(xxx, tSNE_1 > 3.6 & tSNE_1 < 4.6 & tSNE_2 > 1.2 & tSNE_2 < 1.8)), "SD_D-A1_46", "SD_D-A2_39")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro-LRRC17", "Fibro-C7", "Fibro-PLPPR4", "Epi", "SM", "Fibro-Prol", "Glial", "Immune", "Erythrocyte", "Endo", "MSC")
...
# change sort
cell_type_sorted <- c("Epi", "Endo", "SM", "Fibro-LRRC17", "Fibro-PLPPR4", "Fibro-C7", "Fibro-Prol", "Glial", "Immune", "MSC", "Erythrocyte")
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("JD_C"))
...
# dot plot
genes_list <- c("EPCAM", "PECAM1", "ACTA2", "COL1A1", "LRRC17", "PLPPR4", "C7", "TYMS", "S100B", "PTPRC", "PROM1", "HBG1")
...
p$layers <- c(geom_rect(xmin = 4-0.5, xmax = 4+0.5, ymin = 5-0.5, ymax = 8+0.5, fill = "skyblue"), p$layers)
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("SD"))
