# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/10_bronchus/")
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > -3 & tSNE_1 < 0 & tSNE_2 > -6 & tSNE_2 < -1))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (DC)
new_id <- 5
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -3 & tSNE_1 < -1 & tSNE_2 > 2 & tSNE_2 < 9))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (B-Mature)
new_id <- 6
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -15 & tSNE_1 < -9 & tSNE_2 > 5 & tSNE_2 < 12))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Endo)
new_id <- 7
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -4.6 & tSNE_1 < -3.8 & tSNE_2 > -12.5 & tSNE_2 < -11.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (SM)
new_id <- 8
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- setdiff(rownames(subset(xxx, tSNE_1 > -9.5 & tSNE_1 < -6.5 & tSNE_2 > -15 & tSNE_2 < -13)), "JD_C-B5_64")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("T", "B-Immature", "Fibro", "NKT", "Epi", "DC", "B-Mature", "Endo", "SM")
...
# change sort
cell_type_sorted <- c("Epi", "Endo", "SM", "Fibro", "B-Immature", "B-Mature", "DC", "NKT", "T")
...
# dot plot
genes_list <- c("EPCAM", "PECAM1", "ACTA2", "COL1A1", "PTPRC", "CD79A", "IGHM", "IGHD", "ITGAX", "KLRB1", "CD3D")
...
p$layers <- c(geom_rect(xmin = 5-0.5, xmax = 5+0.5, ymin = 1-0.5, ymax = 5+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 4-0.5, ymax = 5+0.5, fill = "skyblue"), p$layers)
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("JD"))
