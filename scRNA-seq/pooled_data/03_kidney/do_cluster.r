...
xxx_cells <- rownames(subset(xxx, tSNE_1 > 36 & tSNE_1 < 38 & tSNE_2 > -10 & tSNE_2 < -6))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 25
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 15 & tSNE_1 < 20 & tSNE_2 > -12.8 & tSNE_2 < -7))
xxx_cells <- c(xxx_cells, "SPZ_C-A4_58")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Endo-TEK", "Podocyte-PLA2R1", "LoH", "PC-CLU", "Epi-Ureter", "Podocyte-GPC3", "Fibro-ALDH1A2", "DT", "Fibro-PTGER2", "LoH-Prog", 
                     "Fibro-GUCY1A2", "PC-BCAT1", "Fibro-CYP1B1", "Epi-Prol", "CM", "PT", "Immune", "Endo-FLT1", "Fibro-COL14A1", "Fibro-APCDD1", 
                     "CACNA1A", "Endo-Glomerular", "SM", "Erythrocyte", "Glial", "Fibro-Prol")
...
# change sort
cell_type_sorted <- c("PT", "LoH", "LoH-Prog", "DT", "PC-CLU", "PC-BCAT1", "Epi-Ureter", "Epi-Prol", "Podocyte-GPC3", "Podocyte-PLA2R1", "CM", 
                      "Endo-TEK", "Endo-FLT1", "Endo-Glomerular", 
                      "SM", "Fibro-ALDH1A2", "Fibro-APCDD1", "Fibro-COL14A1", "Fibro-CYP1B1", "Fibro-PTGER2", "Fibro-GUCY1A2", "Fibro-Prol",
                      "Glial","Immune","CACNA1A","Erythrocyte")
cell_type_sorted
...
gp <- ggplot(enrich_res_DF, aes(x = Term, y = -log10(Adjusted.P.value), fill = new)) + 
  ...
  theme(panel.spacing.y = unit(0.1, "lines"), plot.margin = margin(3,7,-1,7), axis.title.x = element_text(margin = margin(3,0,3,0)), axis.text.y = element_text(size = 10))
gp
dev.off()
#
...
##
p$layers[[3]]$data[5, 3] <- 9
p$layers[[3]]$data[6, 2:3] <- c(-32, -2)
p$layers[[3]]$data[10, 2] <- 1
p$layers[[3]]$data[13, 2] <- 30
p$layers[[3]]$data[14, 2] <- 28
p$layers[[3]]$data[16, 3] <- -20
p$layers[[3]]$data[18, 2] <- 36
p$layers[[3]]$data[20, 2:3] <- c(16, 1.5)
p$layers[[3]]$data[21, 2:3] <- c(37.5, 5)
p$layers[[3]]$data[22, 2:3] <- c(22, -9)
p$layers[[3]]$data[25, 2] <- -20
##
p
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("SPZ_C", "SS_C", "SY_C"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("Cortex", "Medulla", "Pelvis")
  levels(tmp@meta.data$bigBatch)
  ###
...
# dot plot
genes_list <- c("EPCAM", "CUBN", "POU3F3", "PROM1", "SLC8A1", "AQP2", "CLU", "BCAT1", "S100P", "TYMS", 
                "PTPRO", "GPC3", "PLA2R1", "SIX1", 
                "PECAM1", "TEK", "FLT1", "AQP1", 
                "ACTA2", "COL1A1", "ALDH1A2", "APCDD1", "COL14A1", "CYP1B1", "PTGER2", "GUCY1A2", "TOP2A", 
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -15, r = 4, b = 15)), strip.background = element_blank()) + 
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(0, cumsum(tmp_DF$value)[10])),
            aes(x = x, y = y), color = "tomato", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[grep("^Endo-", tmp_DF$ident)[1] - 1], cumsum(tmp_DF$value)[rev(grep("^Endo-", tmp_DF$ident))[1]])),
            aes(x = x, y = y), color = "orange", size = 1.5, inherit.aes = F) +
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("SPZ_C", "SS_C", "SY_C"))

### change levels
levels(tmp_DF$bigBatch)
levels(tmp_DF$bigBatch) <- c("Cortex", "Medulla", "Pelvis")
levels(tmp_DF$bigBatch)
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 1+0.8/2, xend = 2-0.8/2, y = (1-cumsum(Cortex)), yend = (1-cumsum(Medulla))), size = 0.3) + 
  geom_segment(mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = 1, yend = 1), size = 0.3) + 
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = (1-cumsum(Medulla)), yend = (1-cumsum(Pelvis))), size = 0.3) + 
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(0, cumsum(tmp_DF_sub$value)[10])), 
            aes(x = x, y = y), color = "tomato", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Endo-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Endo-", tmp_DF_sub$ident))[1]])), 
            aes(x = x, y = y), color = "orange", size = 1.5, inherit.aes = F) + 
...
p <- DotPlot_new(object = expr_assigned, genes.plot = genes_list, 
                 plot.legend = TRUE, x.lab.rot = T, rev.x = T, rev.y = T, do.plot = F, do.return = T) + 
  theme(legend.position="bottom", legend.justification = "center") + 
  theme(legend.box.margin = margin(t = -10, b = -10, r = 20)) + 
  theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5, linetype = 1))
p$layers <- c(geom_rect(xmin = 1-0.5, xmax = 1+0.5, ymin = 17-0.5, ymax = 26+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 24-0.5, ymax = 25+0.5, fill = "skyblue"), p$layers)
