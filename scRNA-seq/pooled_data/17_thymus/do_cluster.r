# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/17_thymus/")
...
# find another cluster (T-CACNA1A)
new_id <- 4
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -5 & tSNE_1 < 0 & tSNE_2 > -12 & tSNE_2 < -7))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Fibro)
new_id <- 5
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- "XX_C-C5_35"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("T-DP", "Pre-T", "T-Prol", "T-CACNA1A", "Fibro")
...
# change sort
cell_type_sorted <- c("Fibro", "Pre-T", "T-DP", "T-Prol", "T-CACNA1A")
...
# hierarchy marker
FeaturePlot_new(object = expr, features.plot = c("EPCAM", "VIM", "PTPRC", "CD3D"), new.title = c("EPCAM (CD326)", "VIM", "PTPRC (CD45)", "CD3D"), 
                cols.use = c("grey", "blue"), reduction.use = "tsne", pt.size = 1, no.axes = T)

# dot plot
genes_list <- c("COL1A1", "PTPRC", "CD3D", "CD44", "PTPRA", "CD4", "CD8B", "TYMS", "CACNA1A")
...
tmp <- do.call("rbind", lapply(split(tmp, tmp$ident), function(x) { head(x, 3) }))
...
tmp <- tmp[! duplicated(tmp$gene), ]
tmp <- do.call("rbind", split(tmp, tmp$ident))
DoHeatmap_new(object = expr_assigned, genes.use = c("COL1A1", "COL3A1", "COL14A1", tmp$gene), genes.group = factor(c(rep("Fibro", 3), as.character(tmp$ident)), levels = levels(tmp$ident)), slim.col.label = TRUE, remove.key = F, rotate.key = T, 
...
tmp <- subset(expr.markers_ftd_labelled, spec) %>% group_by(cluster) %>% top_n(10, avg_logFC)
tmp <- tmp[order(as.character(tmp$ident)), ]
...
# FeaturePlot
for(i in c("EPCAM", "VIM", "PECAM1", "COL1A1", "IRF1", "FOSB", "CACNA1A", "TYMS", top1$gene)) {
