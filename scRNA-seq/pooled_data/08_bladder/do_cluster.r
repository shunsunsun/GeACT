# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/08_bladder/")
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > 3 & tSNE_1 < 9 & tSNE_2 > -11 & tSNE_2 < -5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 4
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -4 & tSNE_1 < -3 & tSNE_2 > -9 & tSNE_2 < -7))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 5
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -11.5 & tSNE_1 < -11 & tSNE_2 > -3 & tSNE_2 < -2))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 6
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -10 & tSNE_1 < -8 & tSNE_2 > -9 & tSNE_2 < -6))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster
new_id <- 7
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -8 & tSNE_1 < -7.5 & tSNE_2 > 0 & tSNE_2 < 1))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# combine clusters
new_id <- 0
xxx_cells <- names(expr@ident)[expr@ident == 2]
expr@ident[xxx_cells] <- new_id
expr@ident <- factor(expr@ident)
expr@meta.data[xxx_cells, "cluster"] <- new_id
# combine clusters
new_id <- 0
xxx_cells <- names(expr@ident)[expr@ident == 1]
expr@ident[xxx_cells] <- new_id
expr@ident <- factor(expr@ident)
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro", "Immune", "Endo-ENTPD1", "Endo-PODXL", "SM", "Erythrocyte")
...
# change sort
cell_type_sorted <- c("Endo-ENTPD1", "Endo-PODXL", 
                      "SM", "Fibro", 
                      "Immune", "Erythrocyte")
...
p <- TSNEPlot(object = tmp, do.label = TRUE, pt.size = 1, label.size = 3, no.legend = F, do.return = T) +
  scale_color_manual(labels = ident_labels, values = scales::hue_pal()(length(levels(tmp@ident)) + 1)[-1]) + 
  theme(aspect.ratio = 1, axis.line = element_line(color = "black"), panel.border = element_blank()) + 
  #guides(color = guide_legend(ncol = 1)) + 
  coord_cartesian(clip = "off") + 
  xlab("tSNE-1") + ylab("tSNE-2")
###
p$layers[[3]]$data[1, 3] <- -8.8
p$layers[[3]]$data[2, 2:3] <- c(-9.6, -2)
p$layers[[3]]$data[6, 2:3] <- c(-8, 0)
###
p
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("JCN_C", "JCP_C"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("KIT-", "KIT+")
  levels(tmp@meta.data$bigBatch)
  ###
...
# hierarchy marker
FeaturePlot_new(object = expr, features.plot = c("PECAM1", "VIM", "PTPRC", "HBG1"), new.title = c("PECAM1 (CD31)", "VIM", "PTPRC (CD45)", "HBG1"), 
                cols.use = c("grey", "blue"), reduction.use = "tsne", pt.size = 1, no.axes = T)

# dot plot
genes_list <- c("PECAM1", "ENTPD1", "PODXL", "ACTA2", "COL1A1", "PTPRC", "HBG1")
p <- DotPlot_new(object = expr_assigned, genes.plot = genes_list, 
  ...
  theme(legend.box.margin = margin(r = 20)) + 
  theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5, linetype = 1))
p$layers <- c(geom_rect(xmin = 1-0.5, xmax = 1+0.5, ymin = 5-0.5, ymax = 6+0.5, fill = "skyblue"), p$layers)
...
tmp_data_melted$ident <- factor(tmp_data_melted$ident, levels = rev(c("Epi", levels(tmp_data_melted$ident))))
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -7.5, r = 20, b = 15)), strip.background = element_blank()) + 
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("PG"))
...
# FeaturePlot
for(i in c("VIM", "PECAM1", "PROM1", "COL1A1", "IRF1", "FOSB", "CACNA1A", "TYMS", top1$gene)) {
