# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data/11_diaphragm/")
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > -13 & tSNE_1 < -9 & tSNE_2 > -13.5 & tSNE_2 < -9.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Immune)
new_id <- 6
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 4 & tSNE_1 < 14 & tSNE_2 > -12 & tSNE_2 < -7))
xxx_cells <- c(xxx_cells, rownames(subset(xxx, tSNE_1 > 17.25 & tSNE_1 < 17.5 & tSNE_2 > 5.5 & tSNE_2 < 6)), "GJ_C-C2_89")
xxx_cells <- c(xxx_cells, rownames(subset(xxx, tSNE_1 > -13.4 & tSNE_1 < -12.4 & tSNE_2 > -14.5 & tSNE_2 < -13)))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Endo)
new_id <- 7
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- c(rownames(subset(xxx, tSNE_1 > 14 & tSNE_1 < 19 & tSNE_2 > -0.5 & tSNE_2 < 4)), "GJ_C-C1_48")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Glial)
new_id <- 8
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 4.5 & tSNE_1 < 5.5 & tSNE_2 > -4 & tSNE_2 < -2.5))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
# find another cluster (Muscle-Skeletal)
new_id <- 9
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- rownames(subset(xxx, tSNE_1 > -17 & tSNE_1 < -14 & tSNE_2 > -4 & tSNE_2 < -1))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new.cluster.ids <- c("Fibro-SCN7A", "Fibro-GUCY1A3", "Fibro-SFRP2", "SM", "Fibro-COL11A1", "Fibro-Prol", "Immune", "Endo", "Glial", "SKM")
...
# change sort
cell_type_sorted <- c("Endo", "SM", "SKM", "Fibro-SCN7A", "Fibro-GUCY1A3", "Fibro-SFRP2", "Fibro-COL11A1", "Fibro-Prol", "Glial", "Immune")
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("GJ_C"))
...
# dot plot
genes_list <- c("PECAM1", "ACTA2", "ACTA1", "COL1A1", "SCN7A", "GUCY1A3", "SFRP2", "COL11A1", "TYMS", "S100B", "PTPRC")
...
p$layers <- c(geom_rect(xmin = 4-0.5, xmax = 4+0.5, ymin = 3-0.5, ymax = 7+0.5, fill = "skyblue"), p$layers)
...
p <- ggplot(tmp_data_melted, aes(x = ident, y = value, fill = ident, color = ident)) + geom_violin(show.legend = F, scale = "width") + 
  ...
  theme(strip.text.x = element_text(angle = 45, hjust = 1, margin = margin(t = -7.5, r = 10, b = 15)), strip.background = element_blank()) + 
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  # geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[4], cumsum(tmp_DF$value)[9])),
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("GJ"))
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_path(data = data.frame(x = c(1.5, 1.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Fibro-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Fibro-", tmp_DF_sub$ident))[1]])),
            aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) +
  # geom_path(data = data.frame(x = c(1.5, 1.5), y = 1 - c(cumsum(tmp_DF$value)[4], cumsum(tmp_DF_sub$value)[9])), 
  #           aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
