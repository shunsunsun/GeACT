...
setwd("~/lustre/06-Human_cell_atlas/pooled_data_all/17_thymus/")

library("parallel")
suppressMessages(library("arrow"))

# options
dts <- data.frame(stgid = c("pooled_data", "pooled_data_14w"), tsid = gsub(".*/", "", getwd()), stringsAsFactors = F)
#out <- "X"
...
if(file.exists(paste0("../../pooled_data_14w/cblast_result/", ts_id, ".csv"))) {
  cat("> Use aligned embeddings from Cell BLAST.\n")
  ...
  aligned_res <- cblast_res[, c("tSNE1", "tSNE2", "UMAP1", "UMAP2")]
} else if(file.exists("03-expression/merged/cellAlign/Seurat_metaData.txt")) {
  cat("> Use aligned embeddings from Harmony.\n")
  harmony_res <- read.table(file = "03-expression/merged/cellAlign/Seurat_metaData.txt", header = T, sep = "\t", stringsAsFactors = F)
  rownames(harmony_res) <- harmony_res$cell
  aligned_res <- harmony_res[, c("tSNE_1", "tSNE_2", "UMAP_1", "UMAP_2")]
} else {
  cat("> Use original embeddings.\n")
  aligned_res <- cellMeta_final_filtered_plus[, c("tSNE_1", "tSNE_2", "UMAP_1", "UMAP_2")]
  rownames(aligned_res) <- cellMeta_final_filtered_plus$cell
}
colnames(aligned_res) <- c("tSNE_1_ali", "tSNE_2_ali", "UMAP_1_ali", "UMAP_2_ali")
cellMeta_final_filtered_ali <- merge(cellMeta_final_filtered_plus, aligned_res, by.x = "cell", by.y = 0, sort = F)
