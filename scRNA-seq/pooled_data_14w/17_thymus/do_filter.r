...
cutoff_DF <- data.frame(ABratio = 0.9, cleanReads = 0.4*1e6, mpRatio = 0.6, 
                        nGene_l = 1000, nGene_h = 7400, nUMI_l = 3*1e3, nUMI_h = 130*1e3, 
...
cellStat_DF$batch <- gsub("_[0-9][0-9]$", "", rownames(cellStat_DF))
cellStat_DF$batch <- factor(cellStat_DF$batch, levels = unique(cellStat_DF$batch))
ggplot(cellStat_DF, aes(x = batch, y = nGene, fill = batch)) + geom_boxplot(show.legend = F) + 
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
...
cellStat_plus <- cellStat
cellStat_plus$batch <- gsub("_[0-9]+$", "", rownames(cellStat_plus))
ggplot(cellStat_plus, aes(x = batch, y = nGene)) + geom_violin(fill = "skyblue", color = "skyblue") + 
