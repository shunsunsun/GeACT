# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data_14w/17_thymus/")
...
xxx_cells <- rownames(subset(xxx, tSNE_1 > 7 & tSNE_1 < 8 & tSNE_2 > 11.4 & tSNE_2 < 11.8))
...
new_id <- "CACNA1A"
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- setdiff(rownames(subset(xxx, tSNE_1 > -11 & tSNE_1 < -5 & tSNE_2 > 9.5 & tSNE_2 < 12.5)), c("XX_E-B4_65", "XX_E-B4_80"))
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id
...
new_id <- "Erythrocyte"
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), new_id))
xxx_cells <- c("XX_E-B4_52", "XX_E-B2_89", "XX_E-B5_35")
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id

# change ID
new_id <- "0"
xxx_cells <- "XX_E-B2_88"
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id

# change ID
ct_id <- data.frame(old = levels(expr@ident)[1:5], new = c("T-DP", "Pre-T", "T-CACNA1A", "T-Prol", "Fibro"), stringsAsFactors = F)
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident), ct_id$new))
for(i in 1:nrow(ct_id)) {
  xxx_cells <- names(expr@ident)[expr@ident == ct_id$old[i]]
  expr@ident[xxx_cells] <- ct_id$new[i]
  expr@meta.data[xxx_cells, "cluster"] <- ct_id$new[i]
}
rm(ct_id, i)
expr@ident <- factor(expr@ident, levels = c("Endo", "Fibro", "Pre-T", "T-DP", "T-Prol", "T-CACNA1A", "CACNA1A", "Erythrocyte"))

rm(xxx, xxx_cells, new_id)

expr_tmp <- expr
expr_tmp@ident <- factor(expr_tmp@ident)
color_tmp <- ctMeta_20w$color[match(levels(expr_tmp@ident), ctMeta_20w$ident)]
color_tmp[is.na(color_tmp)] <- c("orange", "dodgerblue2", "violetred3")
...
DimPlot(object = expr, reduction.use = "umap", no.legend = F, do.return = TRUE, vector.friendly = TRUE, pt.size = 3) + 
...
expr.markers_spec <- do_findSpecMarker(expr, cl.excluded = NULL, ncpu = 10)
...
color_DF$color[is.na(color_DF$color)] <- c("orange", "dodgerblue2", "violetred3")
...
# dot plot
genes_list <- c("COL1A1", "PTPRC", "CD3D", "CD44", "PTPRA", "CD4", "CD8B", "TYMS", "CACNA1A", "HBG1")
p <- DotPlot_new(object = expr_assigned, genes.plot = genes_list, 
  ...
  theme(legend.box.margin = margin(r = 30)) + 
  theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5, linetype = 1))
p$layers <- c(geom_rect(xmin = 2-0.5, xmax = 2+0.5, ymin = 3-0.5, ymax = 6+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 3-0.5, ymax = 6+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[2], cumsum(tmp_DF$value)[6])),
