# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data_14w/03_kidney/")
...
cell2ident[cell2ident$cell %in% c("SZ_E-A5_73", "SZ_E-A10_91"), "ident"] <- "Erythrocyte"
cell2ident[cell2ident$cell %in% c("SZ_E-A7_70"), "ident"] <- "Fibro-COL14A1"
cell2ident[cell2ident$cell %in% c("SZ_E-A3_53", "SZ_E-A1_96"), "ident"] <- "Unknown"
...
new_id <- "Fibro-SFRP2"
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident)[1:21], new_id, levels(expr@ident)[22:length(levels(expr@ident))]))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 18 & tSNE_1 < 23 & tSNE_2 > -10.5 & tSNE_2 < -8.02))
...
expr.markers_spec <- do_findSpecMarker(expr, ncpu = 10)
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("XCS_E", "ZX_E"))
...
# dot plot
genes_list <- c("EPCAM", "CUBN", "POU3F3", "PROM1", "SLC8A1", "AQP3", "CLU", "BCAT1", "UPK1B", "TYMS", 
                "PTPRO", "GPC3", "PLA2R1", "SIX1", 
                "PECAM1", "TEK", "FLT1", "AQP1", 
                "ACTA2", "COL1A1", "ALDH1A2", "APCDD1", "COL14A1", "CYP1B1", "PTGER2", "GUCY1A2", "SFRP2", "TOP2A", 
                "PLP1", "PTPRC", "CACNA1A", "HBG1")
p <- DotPlot_new(object = expr_assigned, genes.plot = genes_list, 
  ...
  theme(legend.box.margin = margin(t = -10, b = -10, r = 40)) + 
  theme(axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5, linetype = 1))
p$layers <- c(geom_rect(xmin = 1-0.5, xmax = 1+0.5, ymin = 17-0.5, ymax = 26+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 24-0.5, ymax = 25+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 21-0.5, ymax = 22+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 11-0.5, xmax = 11+0.5, ymin = 17-0.5, ymax = 18+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 15-0.5, xmax = 15+0.5, ymin = 13-0.5, ymax = 15+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 20-0.5, xmax = 20+0.5, ymin = 5-0.5, ymax = 11+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[24], cumsum(tmp_DF$value)[25])),
...
# tmp_DF <- melt(tmp / rowSums(tmp))
# # sort by real position
# tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("XCS_E", "ZX_E"))
# 
# ### change levels
# levels(tmp_DF$bigBatch)
...
#   geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 1+0.8/2, xend = 2-0.8/2, y = (1-cumsum(Upper)), yend = (1-cumsum(`Middle/Lower`))), size = 0.3) + 
