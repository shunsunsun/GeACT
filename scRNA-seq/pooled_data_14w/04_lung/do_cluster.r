# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data_14w/04_lung/")
...
cell_tmp <- c("FZY_E-C5_91","FZY_E-C5_07","FZY_E-C1_53","FZY_E-C3_32","FZY_E-C5_75",
              "FZY_E-C3_82","FZY_E-C1_25","FZY_E-C3_87","FZY_E-C1_72","FZY_E-C4_04",
              "FZY_E-C3_62","FZY_E-C3_39","FZY_E-C5_53","FZW_E-B2_46","FZW_E-B3_45",
              "FZW_E-B5_77","FZW_E-B5_73","FZW_E-B2_56","FZW_E-B4_81","FZW_E-B3_21",
              "FZW_E-B4_42","FZW_E-B5_25","FZW_E-B1_60","FZW_E-B5_05","FZW_E-B5_93",
              "FZW_E-B2_21","FZW_E-B5_07","FZW_E-B2_37","FZW_E-B3_80","FZW_E-B5_56")
cell2ident[cell2ident$ident == "Fibro-FOSB" & (! cell2ident$cell %in% cell_tmp), "ident"] <- "Fibro-0"
cell2ident[cell2ident$cell %in% cell_tmp, "ident"] <- "Fibro-FOSB"
cell2ident[cell2ident$cell %in% c("FZW_E-B4_62", "FZY_E-C3_60"), "ident"] <- "Fibro-TNC"
cell2ident[cell2ident$ident == "SM-STC1" & (! cell2ident$cell %in% c("FZW_E-B3_59", "FZW_E-B5_68", "FZY_E-C3_16")), "ident"] <- "Fibro-0"
cell2ident[cell2ident$cell %in% c("FZW_E-B1_78"), "ident"] <- "SM-HHIP"
cell2ident[cell2ident$cell %in% c("FZY_E-C2_42"), "ident"] <- "Erythrocyte"
cell2ident[cell2ident$cell %in% c("FZY_E-C2_73"), "ident"] <- "Unknown"
...
new_id <- "SM-0"
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident)[1:4], new_id, levels(expr@ident)[5:length(levels(expr@ident))]))
xxx_cells <- names(expr@ident)[expr@ident == "SM-STC1"]
expr@ident[xxx_cells] <- new_id
expr@meta.data[xxx_cells, "cluster"] <- new_id

# # change cluster ID
# new_id <- "Podocyte-PLA2R1"
# xxx_cells <- rownames(subset(xxx, tSNE_1 > -8 & tSNE_1 < -5.5 & tSNE_2 > 17.5 & tSNE_2 < 20.5))
# expr@ident[xxx_cells] <- new_id
# expr@meta.data[xxx_cells, "cluster"] <- new_id
# 
# # change cluster ID
# new_id <- "SM-0"
# xxx_cells <- intersect(names(expr@ident)[expr@ident == "Podocyte-GPC3"], rownames(subset(xxx, ! (tSNE_1 > -9 & tSNE_1 < -4 & tSNE_2 > 17 & tSNE_2 < 21))))
# expr@ident[xxx_cells] <- new_id
# expr@meta.data[xxx_cells, "cluster"] <- new_id

rm(xxx, xxx_cells)

# # ident result comparison
# tmp <- expr@meta.data[, c("res.0.6", "cluster")]
# tmp$cluster <- factor(tmp$cluster, levels = levels(expr@ident))
# ct_confusion_MT <- acast(tmp, res.0.6 ~ cluster, value.var = "cluster", fun.aggregate = length)
# ct_confusion_MT_ratio_1 <- sweep(ct_confusion_MT, MARGIN = 1, STATS = rowSums(ct_confusion_MT), FUN = "/")
# ct_confusion_MT_ratio_2 <- sweep(ct_confusion_MT, MARGIN = 2, STATS = colSums(ct_confusion_MT), FUN = "/")
# pheatmap(ct_confusion_MT_ratio_1, cluster_rows = T, cluster_cols = F, display_numbers = T)
# pheatmap(ct_confusion_MT_ratio_2, cluster_rows = T, cluster_cols = F, display_numbers = T)

expr_tmp <- expr
expr_tmp@ident <- factor(expr_tmp@ident)
color_tmp <- ctMeta_20w$color[match(levels(expr_tmp@ident), ctMeta_20w$ident)]
color_tmp[is.na(color_tmp)] <- c("#CAFF70", "#B3B3B3")
gp <- TSNEPlot(object = expr_tmp, pt.size = 2, do.label = T, no.legend = T, colors.use = color_tmp, do.return = T)
ggsave(filename = paste0(OUT, "/Seurat_tSNE_after_correction2.pdf"), plot = gp, width = 6, height = 6, useDingbats = F)
rm(expr_tmp, color_tmp)
###
...
DimPlot(object = expr, reduction.use = "umap", no.legend = F, do.return = TRUE, vector.friendly = TRUE, pt.size = 3, cols.use = c(scales::hue_pal()(length(levels(expr@ident)) - 1), "#B3B3B3")) + 
  ggtitle("UMAP") + theme(plot.title = element_text(hjust = 0.5)) + guides(color = guide_legend(ncol = 1))
...
cellMetaData$cluster <- factor(cellMetaData$cluster, levels = levels(expr@ident))
...
expr.markers_pn <- do_findMarker(expr, only.pos = F, ncpu = 10)
...
expr.markers_ftd <- subset(expr.markers_ftd, cluster != "Unknown")
...
expr.markers_spec <- do_findSpecMarker(expr, cl.excluded = "Fibro-0", ncpu = 10)
expr.markers_spec <- subset(expr.markers_spec, cluster != "Unknown")
expr.markers_spec$cluster <- factor(expr.markers_spec$cluster)
...
id_DF <- data.frame(old = levels(expr@ident), new = levels(expr@ident), stringsAsFactors = F)
id_DF
cell_type_sorted <- levels(expr@ident)
...
expr.markers_ftd_labelled$ident <- factor(expr.markers_ftd_labelled$ident)
...
# 
# # average cor
# cl <- makeCluster(min(length(levels(expr@ident)), 10))
...
## set color
color_DF <- data.frame(ident = levels(expr@ident), tissue = ts_id, stringsAsFactors = F)
color_DF$color <- ctMeta_20w[match(color_DF$ident, ctMeta_20w$ident), "color"]
color_DF$color[is.na(color_DF$color)] <- c("#CAFF70", "#B3B3B3")
write.table(x = color_DF, file = paste0(OUT, "/color_DF.txt"), row.names = F, col.names = F, quote = F, sep = "\t")
##
...
p <- TSNEPlot(object = tmp, do.label = TRUE, pt.size = 1, label.size = 3, no.legend = F, do.return = T) +
  scale_color_manual(labels = ident_labels, values = color_DF$color[match(levels(tmp@ident), color_DF$ident)]) + 
...
###
#p$layers[[3]]$data[p$layers[[3]]$data$ident == "SM-Vascular", 2] <- -27
p$layers[[3]]$data <- p$layers[[3]]$data[p$layers[[3]]$data$ident != "Unknown", ]
###
p
#ggsave(filename = "test.pdf", plot = p, width = 6, height = 6, useDingbats = F)
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("FZY_E", "FZW_E"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("Central", "Peripheral")
  levels(tmp@meta.data$bigBatch)
  ###
...
# hierarchy marker
FeaturePlot_new(object = expr, features.plot = c("EPCAM", "VIM", "PECAM1", "HBG1"), new.title = c("EPCAM (CD326)", "VIM", "PTPRC (CD45)", "HBG1"), 
                cols.use = c("grey", "blue"), reduction.use = "tsne", pt.size = 1, no.axes = T)

# dot plot
genes_list <- c("EPCAM", "PECAM1", 
                "ACTA2", "STC1", "HHIP", "COL1A1", "FOSB", "TNC", "TYMS", "TRPC6", 
                "CD4", "HBG1")
...
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 8-0.5, ymax = 9+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 4-0.5, ymax = 7+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[10], cumsum(tmp_DF$value)[11])),
...
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("FZY_E", "FZW_E"))
### change levels
levels(tmp_DF$bigBatch)
levels(tmp_DF$bigBatch) <- c("Central", "Peripheral")
levels(tmp_DF$bigBatch)
###
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") +
  ...
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 1+0.8/2, xend = 2-0.8/2, y = (1-cumsum(Central)), yend = (1-cumsum(Peripheral))), size = 0.3) +
  # geom_segment(mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = 1, yend = 1), size = 0.3) +
  # geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 2+0.8/2, xend = 3-0.8/2, y = (1-cumsum(Body)), yend = (1-cumsum(Antrum))), size = 0.3) +
  geom_path(data = data.frame(x = c(2.5, 2.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Fibro-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Fibro-", tmp_DF_sub$ident))[1]])),
            aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) +
  geom_path(data = data.frame(x = c(2.5, 2.5), y = 1 - c(cumsum(tmp_DF_sub$value)[14], cumsum(tmp_DF_sub$value)[18])),
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) +
...
DoHeatmap_new(object = expr_assigned, genes.use = tmp$gene, genes.group = tmp$ident, slim.col.label = TRUE, remove.key = F, rotate.key = T,
              group.cex = 11, cex.row = 7, do.colBar = T, colBar.col = color_DF$color[match(levels(tmp$ident), color_DF$ident)], strip.text.x.top = 15,
              panel.spacing.y = 0,
              cells.use = WhichCells(expr_assigned, ident = intersect(setdiff(levels(expr_assigned@ident), NA), levels(tmp$ident)), max.cells.per.ident = 80, random.seed = 1))
...
DoHeatmap_new(object = expr_assigned, genes.use = cyc_DF$gene, genes.group = factor(cyc_DF$stage, levels = c("S", "G2/M")), slim.col.label = TRUE, remove.key = F, rotate.key = T,
              group.cex = 11, cex.row = 7, do.colBar = T, colBar.col = color_DF$color, switch_y = T, strip.text.x.top = 15,
              col.low = "blue", col.mid = "white", col.high = "red",
...
write.table(x = expr_assigned@dr$umap@cell.embeddings, file = paste0(OUT, "/Seurat_UMAP_embeddings.txt"), row.names = T, col.names = NA, quote = F,sep = "\t")
