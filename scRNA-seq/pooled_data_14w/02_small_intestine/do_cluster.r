...
cell2ident[cell2ident$cell %in% c("XCS_E-N4_31", "XCS_E-N4_73", "XCS_E-N6_02", "XCS_E-N4_22", "XCS_E-N4_46", "XCS_E-N4_19"), "ident"] <- "Unknown"  # high HBG1
cell2ident[cell2ident$cell %in% c("ZX_E-N24_03", "ZX_E-N7_39", "ZX_E-N30_91"), "ident"] <- "Unknown"  # no CD3D
...
new_id <- "Fibro-SCARA5"
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident)[1:9], new_id, levels(expr@ident)[10:length(levels(expr@ident))]))
xxx_cells <- intersect(names(expr@ident)[expr@ident == "Fibro-COL6A5"], rownames(subset(xxx, tSNE_1 > 4 & tSNE_2 < 13)))
...
TSNEPlot(object = expr, pt.size = 2, do.label = T, no.legend = T, colors.use = c(scales::hue_pal()(length(levels(expr@ident)) - 1), "#B3B3B3"))
...
p$layers[[3]]$data[p$layers[[3]]$data$ident == "Fibro-KCNN3", 2:3] <- list(14, -29.5)
p$layers[[3]]$data[p$layers[[3]]$data$ident == "Fibro-CXCL14", 2] <- -8.5
p$layers[[3]]$data[p$layers[[3]]$data$ident == "Fibro-ZEB1", 2:3] <- list(11, 30.5)
p$layers[[3]]$data[p$layers[[3]]$data$ident == "T", 2:3] <- list(28.2, -15)
p$layers[[3]]$data[p$layers[[3]]$data$ident == "T-Prog", 2:3] <- list(34, -18.5)
...
genes_list <- c("EPCAM", "PECAM1", "ACTA2", "ACTG2", "NOTCH3", 
                "PDGFRA", "COL14A1", "GPC3", "VCAM1", "KCNN3", "COL6A5", "SCARA5", "CXCL14", "ZEB1", "TYMS", "PLP1", "PTPRC", "MS4A1", "CD68", "CD3D", "KIT", "CACNA1A", "HBG1")
...
p$layers <- c(geom_rect(xmin = 3-0.5, xmax = 3+0.5, ymin = 18-0.5, ymax = 19+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 9-0.5, ymax = 17+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 17-0.5, xmax = 17+0.5, ymin = 4-0.5, ymax = 7+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 20-0.5, xmax = 20+0.5, ymin = 4-0.5, ymax = 5+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[14], cumsum(tmp_DF$value)[18])),
...
DoHeatmap_new(object = expr_assigned, genes.use = tmp$gene, genes.group = tmp$ident, slim.col.label = TRUE, remove.key = F, rotate.key = T,
              group.cex = 11, cex.row = 7, do.colBar = T, colBar.col = color_DF$color, strip.text.x.top = 15,
              panel.spacing.y = 0,
