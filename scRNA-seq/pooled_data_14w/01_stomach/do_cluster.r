# cell classification
setwd("~/lustre/06-Human_cell_atlas/pooled_data_14w/01_stomach/")
...
expr@ident <- factor(cell2ident$ident, levels = c(cellType_20w, setdiff(unique(cell2ident$ident), cellType_20w)))
names(expr@ident) <- cell2ident$cell
expr@meta.data$cluster <- cell2ident$ident

# find another cluster
new_id <- "Fibro-KIT"
expr@ident <- factor(expr@ident, levels = c(levels(expr@ident)[1:10], new_id, levels(expr@ident)[11:20]))
xxx_cells <- rownames(subset(xxx, tSNE_1 > 12 & tSNE_1 < 16 & tSNE_2 > -40 & tSNE_2 < -36))
...
p$layers[[3]]$data[p$layers[[3]]$data$ident == "CACNA1A-ELF3-", 2:3] <- list(-2, 26)
...
if(length(unique(expr@meta.data$bigBatch)) > 1) {
  ### change levels
  tmp@meta.data$bigBatch <- factor(tmp@meta.data$bigBatch, levels = c("WD_E", "WT_E", "WDOU_E"))
  levels(tmp@meta.data$bigBatch)
  levels(tmp@meta.data$bigBatch) <- c("Fundus", "Body", "Antrum")
  levels(tmp@meta.data$bigBatch)
  ###
...
genes_list <- c("EPCAM", "PECAM1", "ACTA2", "ACTG2", "NOTCH3", 
                "PDGFRA", "KCNJ8", "ADAM28", "IRF1", "VSTM2A", "FBLN1", "NRK", "KIT", "PRSS35", "TYMS", "PLP1", "PTPRC", "CD79A", "CD68", "CD3D", "CACNA1A", "ELF3", "HBG1")
...
p$layers <- c(geom_rect(xmin = 6-0.5, xmax = 6+0.5, ymin = 9-0.5, ymax = 16+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 17-0.5, xmax = 17+0.5, ymin = 5-0.5, ymax = 7+0.5, fill = "skyblue"), p$layers)
p$layers <- c(geom_rect(xmin = 21-0.5, xmax = 21+0.5, ymin = 3-0.5, ymax = 4+0.5, fill = "skyblue"), p$layers)
...
ggplot(tmp_DF, aes(x = factor(1), y = value, fill = ident)) + 
  ...
  geom_path(data = data.frame(x = c(1.55, 1.55), y = 1 - c(cumsum(tmp_DF$value)[14], cumsum(tmp_DF$value)[17])),
...
tmp_DF <- melt(tmp / rowSums(tmp))
# sort by real position
tmp_DF$bigBatch <- factor(tmp_DF$bigBatch, levels = c("WD_E", "WT_E", "WDOU_E"))

### change levels
levels(tmp_DF$bigBatch)
levels(tmp_DF$bigBatch) <- c("Fundus", "Body", "Antrum")
levels(tmp_DF$bigBatch)
...
ggplot(tmp_DF, aes(x = bigBatch, y = value, fill = ident)) + geom_bar(stat = "identity", width = 0.8, color = "white") + 
  ...
  geom_segment(data = dcast(tmp_DF, ident ~ bigBatch), mapping = aes(x = 1+0.8/2, xend = 2-0.8/2, y = (1-cumsum(Fundus)), yend = (1-cumsum(Body))), size = 0.3) + 
  ...
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[grep("^Fibro-", tmp_DF_sub$ident)[1] - 1], cumsum(tmp_DF_sub$value)[rev(grep("^Fibro-", tmp_DF_sub$ident))[1]])), 
            aes(x = x, y = y), color = "#00BF74", size = 1.5, inherit.aes = F) + 
  geom_path(data = data.frame(x = c(3.5, 3.5), y = 1 - c(cumsum(tmp_DF_sub$value)[14], cumsum(tmp_DF_sub$value)[17])), 
            aes(x = x, y = y), color = "mediumpurple1", size = 1.5, inherit.aes = F) + 
