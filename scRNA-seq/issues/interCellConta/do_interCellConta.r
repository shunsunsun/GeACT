...
setwd("~/lustre/06-Human_cell_atlas/F1_DT1_alt/")
OUT <- "03-expression/merged/deConta"
...
deConta_res <- parLapply(cl, umiCell_LS, function(x) {  # for each gene
  ...
  if(nrow(x1) == 0) {
    ...
    stat_res_fm <- data.frame(row.names = colnames(x), 
                              contaRow = rep(0, ncol(x)), contaCol = rep(0, ncol(x)), contaOth = rep(0, ncol(x)))
  } else {
    x1_res_LS <- apply(x1, 1, function(xi) {  # for each UMI
      ...
      if(nrow(diff_DF) > 0) {
        ...
        diff_DF$conta <- apply(diff_DF, 1, function(x) {
          if(x[5]!=x[7] && x[6]!=x[8]) { y = "contaOth" }
          else if(x[6]==x[8]) { y = "contaRow" }  # same inner
          else if(x[5]==x[7]) { y = "contaCol" }  # same outer
          return(y)
        })
      }
    ...
    x1_res <- t(sapply(x1_res_LS, "[[", 1))
    x_res <- rbind(x1_res, x[apply(x>0, 1, sum) == 1, ])
    
    stat_res <- do.call("rbind", lapply(x1_res_LS, "[[", 2))
    if(nrow(stat_res) > 0) {
  ...
  # add origin
  stat_res_fi <- merge(data.frame(total = colSums(x)), stat_res_fm, by = 0, sort = F, all.x = T)
  stat_res_fi[is.na(stat_res_fi)] <- 0
  stat_res_fi$origin <- stat_res_fi$total - rowSums(stat_res_fi[, c("contaRow", "contaCol", "contaOth")])
...
deConta_expr_LS <- lapply(tab_DF_sub$cell, function(cs) { # for each cell
  ...
  ys <- parLapply(cl, deConta_res, function(x) {  # for each gene
    ### x <- deConta_res[[1]]
    if(cs %in% colnames(x[[1]])) {
      y <- x[[1]][, cs, drop = F]
      y <- y[y[, 1] > 0, , drop = F]
      y <- data.frame(UMI = rownames(y), num = y[, 1], stringsAsFactors = F)
    } else {
      y <- data.frame(UMI = character(), num = integer(), stringsAsFactors = F)
    }
    return(y)
  })
...
for(i in seq_along(deConta_expr_LS)) {
  cs <- tab_DF_sub$cell[i]
  cat(">", cs, "\n")
  write.table(x = deConta_expr_LS[[i]], file = paste0(OUT, "/", batch_id, "/", cs, "_htseq.tab"), 
...
deConta_stat_agg <- aggregate(x = deConta_stat_DF[, 2:6], by = list(cell = deConta_stat_DF$cell), FUN = sum)
deConta_stat_agg$ratioRow <- deConta_stat_agg$contaRow / deConta_stat_agg$total
deConta_stat_agg$ratioCol <- deConta_stat_agg$contaCol / deConta_stat_agg$total
deConta_stat_agg$ratioOth <- deConta_stat_agg$contaOth / deConta_stat_agg$total

write.table(x = deConta_stat_agg, file = paste0(OUT, "/", batch_id, "/", "deConta_stat.txt"), 
            row.names = F, col.names = T, quote = F, sep = "\t")
