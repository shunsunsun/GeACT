...
eLableAttr <- function(g_in, coords_in, ml_ratio = 0.3, ol_ratio = 0.1, ol_len = 0, loop_adj_x = 0.3, loop_adj_y = 0.1) {
  edge.coords_ot <- t(sapply(strsplit(as_ids(E(g_in)), "\\|"), function(x) {
    id_sta <- x[1]
    id_end <- x[2]
    if(id_sta != id_end) {
      x_dist <- coords_in[id_end, 1] - coords_in[id_sta, 1]
      y_dist <- coords_in[id_end, 2] - coords_in[id_sta, 2]
      l_dist <- sqrt(x_dist^2 + y_dist^2)
      o_dist <- l_dist * ol_ratio + ol_len
      pos_x <- coords_in[id_sta, 1] + x_dist * ml_ratio - o_dist * y_dist / l_dist
      pos_y <- coords_in[id_sta, 2] + y_dist * ml_ratio + o_dist * x_dist / l_dist
      srt_t <- atan(y_dist / x_dist) * 180 / pi
    } else {
      pos_x <- coords_in[id_sta, 1] + loop_adj_x
      pos_y <- coords_in[id_sta, 2] + loop_adj_y
      srt_t <- 0
    }
    return(c(pos_x, pos_y, srt_t))
  }))
  edge.coords_ot <- as.data.frame(edge.coords_ot)
  rownames(edge.coords_ot) <- as_ids(E(g_in))
  colnames(edge.coords_ot) <- c("x", "y", "srt")
  return(edge.coords_ot)
}

plot.igraph_new <- function (x, axes = FALSE, add = FALSE, xlim = c(-1, 1), ylim = c(-1, 
                                                                                                                                                                     ...
                                                                                                                                                                     alpha = 1), mark.expand = 15, ...) 
{
  ...
  edge.label.srt <- params("edge", "label.srt") # XXX
  ...
  loop.label.color <- edge.label.color[loops.e] # XXX
  ...
  if (length(loops.e) > 0) {
    ...
    loop <- function(x0, y0, cx = x0, cy = y0, color, angle = 0, 
                     label = NA, label.color, width = 1, arr = 2, lty = 1, arrow.size = arrow.size, # XXX
                     arr.w = arr.w, lab.x, lab.y) {
      rad <- angle
      center <- c(cx, cy)
      cp <- matrix(c(x0, y0, x0 + 0.2, y0 + 0.2, x0 + 0.2, # XXX
      ...
      if (is.language(label) || !is.na(label)) {
        ...
        text(lx, ly, label, col = label.color, font = edge.label.font, # XXX
    ...
    mapply(loop, xx0, yy0, color = ec, angle = -la, label = loop.labels, label.color = loop.label.color, # XXX
  ...
  if (length(x0) != 0) {
    ...
    if (length(edge.label.srt) > 1) { # XXX
      edge.label.srt <- edge.label.srt[nonloops.e]
    }
    if (length(edge.label.color) > 1) { # XXX
      edge.label.color <- edge.label.color[nonloops.e]
    }
    if (length(arrow.mode) > 1) {
    ...
    if(length(unique(edge.label.srt)) == 1) { # XXX
      text(lc.x, lc.y, labels = edge.labels, col = edge.label.color, srt = edge.label.srt, 
           family = edge.label.family, font = edge.label.font, 
           cex = edge.label.cex)
    } else {
      for(el_i in seq_along(lc.x)) {
      text(lc.x[el_i], lc.y[el_i], labels = edge.labels[el_i], col = edge.label.color[el_i], srt = edge.label.srt[el_i], 
           family = edge.label.family, font = edge.label.font, 
           cex = edge.label.cex)
      }
      rm(el_i)
...
environment(plot.igraph_new) <- asNamespace('igraph')
