...
#print "$UMI\t$polyT\t$cDNA\t$t_num\n";
		if($t_num < $min_t) {
			return ("NA", "NA", "NA", -2, -2, "NA");
		}

		my ($mm_res, $nn_res, $uflag) = &UMI_match($UMI);
		if($mm_res < 3) {
			return ($UMI, $polyT, $cDNA, $mm_res, $nn_res, $uflag);
		} else {
			return ("NA", "NA", $cDNA, $mm_res, $nn_res, $uflag);
		}
	} else {
		return ("NA", "NA", "NA", -1, -1, "NA");
	}
}
...
#print "$i1, $i2, $barcode_seq, $mm_idx\n";
	if($i2 eq "NA") {	# not allow mismatch in both outer and inner barcode (performed above)
		...
		my ($UMI, $polyT, $kept_seg, $mm_idx, $nn_idx, $mm_flag) = &extract_umis($reap{sequence}, 20, 4);	# read2
		if($mm_flag eq "A") {
			$primerA_num{$sample} ++;
			$primerA_num ++;
		} elsif($mm_flag eq "B") {
		...
		# estimate error rate
		if( ($mm_idx >= 0) && ($mm_idx <= 5) && ($nn_idx > 5) ) {
#print ">>> read base number: $mm_idx\n";
			$total_ER ++;
			$observed_ER += ($mm_idx / 20);
		}
...
print (("-" x 40) . "\n");
print "Error rate:\t$total_ER\t$observed_ER\t" . $observed_ER/$total_ER . "\n";
#print "OK.\n";
}
