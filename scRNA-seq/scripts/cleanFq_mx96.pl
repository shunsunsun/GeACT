...
if(@ARGV!=6) {
	...
	print "Example: perl $0 test/test_R1.fastq.gz test/test_R2.fastq.gz test/barcode2sample.txt cleanFq 0 151\n";
	exit;
}
...
if( (@outer_barcodes == 1) && ($outer_barcodes[0] eq "NA") ) {
	print "Usage build-in outer barcodes\n";
	@outer_barcodes = qw /GATATG ATACG CCGTCTG TGCG GAACTCG ATGTAG CCCG TGTAG GAGTAAG ATCG CCTAG TGACCG/;
}
...
#print "$i1, $i2, $barcode_seq, $mm_idx\n";
	if($i2 eq "NA") {	# not allow mismatch in both outer and inner barcode (performed above)
		...
		my $bcds = "NA" . "__" . $inner_barcodes[$i2];	# XXX
