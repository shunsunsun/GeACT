...
do_hc <- function(res_in, use.abs = F, method = "average") {
  cl <- makeCluster(min(length(res_in), 10), type = "FORK")
...
do_detectModule <- function(res_in, method = "constant", h_cutoff, size_cutoff = 10) {
  # read gene type info
  gene_type <- read.table("/rd/user/tianf/06-Human_cell_atlas/Genomes/human/gene_type_class_rmdup.txt", header = F, sep = "\t", stringsAsFactors = F, row.names = 2)
...
do_plotExprHeatmap <- function(res_in, ctype1, ctype2 = NULL, mdid = NULL, mp_in, mpid = NULL, subsp = 500, seed = 1, do.log = T, scale = "row", 
                               do_plot = T, vmin = -2.5, vmax = 2.5, 
                               main = NA, show_rownames = F, show_colnames = F, fontsize_row = 6, fontsize_col = 6, show.legend = T, 
                               do.print = T, do.return = T) {
  if(! is.null(mdid)) {
    ct1 <- res_in[[ctype1]][["cl_table_ftd"]]
  ...
  if(do_plot) {
    ...
    p <- ggplot(expr_comb_melted, aes(x = Var2, y = Var1, fill = value)) + geom_tile(show.legend = show.legend) + 
      scale_fill_gradientn(colours = colorRampPalette(c("purple3", "white","palegreen3"))(100), limits = c(vmin, vmax), breaks = c(vmin, 0, vmax)) + 
      scale_x_discrete(position = "top") + labs(fill = "Expression") + xlab(NULL) + ylab(NULL) + ggtitle(main) + 
      theme(axis.text = element_blank(), axis.ticks = element_blank(), axis.line = element_blank()) + 
      theme(legend.margin = margin(l = -5), aspect.ratio = 2)
    if(! is.null(ctype2)) {
      p <- p + facet_grid(. ~ ct, scales = "free_x", switch = "x") + theme(strip.background = element_blank())
    }
    if(show_rownames) {
      p <- p + theme(axis.text.y = element_text(size = fontsize_row))
    }
...
do_enrich <- function(res_in, ctype = NULL, mdid = NULL, db, category = NULL, expr_cutoff = NULL, ncpu = 1) {
  # read gene sets
  if(is.null(category)) {
...
do_plotEnrich <- function(res_in, ctype, mdid, db = "GO", category = NULL, shortName = T, charMaxLen = NULL, collapse = F, showNum = 10, 
                          main = "auto", show_size = T, show.legend = F) {
  enrich_res <- res_in[[ctype]][[paste0("enrich_", db)]]
  ...
  enrich_res$Description_alt <- enrich_res$Description
  if(shortName) {
    enrich_res$Description_alt <- gsub(",.*", "", enrich_res$Description_alt)
  }
  if(! is.null(charMaxLen)) {
    enrich_res$Description_alt <- substr(enrich_res$Description_alt, start = 1, stop = charMaxLen)
  }
  if(collapse) {
    enrich_res <- enrich_res[! duplicated(enrich_res$Description_alt), ]
  }
  enrich_res <- head(enrich_res, showNum)
  #enrich_res$Aspect <- factor(enrich_res$Aspect, levels = c("P", "F", "C"))
  ...
  if(nrow(enrich_res) == 0) {
    ...
    gp <- ggplot(enrich_res, aes(x = Description, y = -log10(p.adjust), fill = "1")) + geom_bar(stat = "identity", width = 0.9, show.legend = show.legend) + 
      coord_flip() + scale_fill_manual(values = "#4DBBD5B2") + 
      xlab(paste(db, "terms")) + ylab(expression(paste(-log[10], " (FDR)"))) + geom_hline(yintercept = -log10(0.05), linetype = "dashed") + 
      scale_x_discrete(labels = rev(enrich_res$Description_alt)) + scale_y_continuous(expand = c(0, 0)) + 
...
do_prepPPI <- function() {
  ...
  string_db <- STRINGdb$new(version="10", species=9606, score_threshold=0, input_directory=dbDir)
  string_db$load()
...
do_mergeModule <- function(res_in, ov_cutoff = 0.9, verbose = F) {
